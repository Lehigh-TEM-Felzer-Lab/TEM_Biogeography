...
{	
       ...
       FILE* cruarea;
        cruarea = fopen("/m2/partition2/tem/CRU4.04/cru_ts4.04.1901lf.vap.dat_usa48.csv", "r");
        if (!cruarea){
                cerr << "cru area file could not be opened\n";
                exit(1);
        }

        sprintf(file,"vpr_future_cesm_year.usa48");
        ifstream ffut;
	...
	sprintf(file,"vpr_ssp370_2015_2099_tem.usa48");
	ofstream fout; 
