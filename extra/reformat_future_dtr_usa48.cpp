...
{	
        ...
        FILE* crutair;
        crutair = fopen("/m2/partition2/tem/CRU4.04/cru_ts4.04.1901.2014lf.dtr.dat_usa48.csv", "r");
        if (!crutair){
       ...
       FILE* cruarea;
        cruarea = fopen("/m2/partition2/tem/CRU4.04/cru_ts4.04.1901lf.dtr.dat_usa48.csv", "r");
        if (!crutair){
                cerr << "cru area file could not be opened\n";
                exit(1);
        }

        sprintf(file,"dtr_future_cesm_year.usa48");
        ifstream ffut;
	...
	sprintf(file,"dtr_ssp370_2015_2099_tem.usa48");
	ofstream fout; 
...
/*       cout << "final = " << grid << " " << lon[grid] << " " << lat[grid] << endl;
       ...
       if(m < 2100)
       {
       fout << setprecision(1) << lon[grid] << "," << lat[grid] << ", " << "DTAIRRNG" << " ," <<
