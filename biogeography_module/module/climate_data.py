...
temperature_data = dependencies.pd.read_csv(
    temperature_data_path, names=climate_data_columns
)
precipitation_data = dependencies.pd.read_csv(
    precipitation_data_path, names=climate_data_columns
)
...
days_in_month_arr = dependencies.np.array(
    [
        dependencies.calendar.monthrange(year, month)[1]
        for year in years
        for month in months
    ]
).reshape(-1, 12)
temperature_data[
    [
        f"{dependencies.calendar.month_name[month][:3].upper()}_DAYS"
...
monthly_gdd = (
    dependencies.np.maximum(temperature_monthly_data - 5, 0) * days_in_month_arr
)
temperature_data[
    [
        f"{dependencies.calendar.month_name[month][:3].upper()}_GDD"
        for month in range(1, 13)
    ]
] = monthly_gdd
