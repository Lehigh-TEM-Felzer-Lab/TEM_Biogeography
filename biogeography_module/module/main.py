...
def main():
    try:
        ...
        if len(lon_unique) > 1 and len(lat_unique) > 1:
            ...
            print(
                f"{BOLD + BLUE}Data resolution: {RESET}  {BOLD+ CYAN} {lon_resolution} x {lat_resolution} degrees {RESET}"
            )
        else:
        ...
        print(
            BOLD
            + BLUE
            + "Number of TEM output data to be processed: "
            + MAGENTA
            + f"{len(var_list)}"
            + RESET
        )
        ...
        print(
            BOLD
            + YELLOW
            + "Preparing climate datasets and merging with TEM output files..."
            + RESET
        )
        print()
        ...
        print(
            BOLD
            + YELLOW
            + "Determining possible PFTs based on climate in each gridcell..."
            + RESET
        )
        print()
        ...
        for var, df in bakeoff_variables.items():
            result_out[var] = determine_possible_pft(
                df, bakeoff_variables[var]
            )  # Pass the correct dataframe here
        ...
        for var, df in cleaned_bakeoff_result.items():
            export_to_csv(
                ...
                dependencies.os.path.join(
                    bakeoff_results_dir_path, f"{var}_bakeoff_result.csv"
                ),
            )
        ...
        bakeoff_result = dependencies.pd.read_csv(
            npp_bakeoff_results_path, names=var_cols
        )
