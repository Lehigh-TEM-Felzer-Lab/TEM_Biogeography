...
from columns import TEM_OUTPUT_COLUMNS,TEM_CLIMATE_COLUMNS,MEAN_SUMMER_MOISTURE_STRESS_COLUMNS,CLIMATE_LIMITS_COLUMNS,FIRE_COLUMNS

if not os.path.exists("shift.results"):
    open("pca.results", "w").close()

def load_vegc(path):
    vegc = pd.read_csv(path, names=TEM_OUTPUT_COLUMNS)
    vegc.rename(columns={"TOTAL": "TOTAL_VEGC", "MAX": "MAX_VEGC", "AVE": "AVE_VEGC", "MIN": "MIN_VEGC"}, inplace=True)
    vegc = vegc[["LON", "LAT", "POTVEG", "YEAR", "TOTAL_VEGC", "MAX_VEGC", "AVE_VEGC", "MIN_VEGC"]]
    return vegc
...
def load_npp(path):
    npp = pd.read_csv(path, names=TEM_OUTPUT_COLUMNS)
    npp.rename(columns={"TOTAL": "TOTAL_NPP", "MAX": "MAX_NPP", "AVE": "AVE_NPP", "MIN": "MIN_NPP"}, inplace=True)
    npp = npp[["LON", "LAT", "POTVEG", "YEAR", "TOTAL_NPP", "MAX_NPP", "AVE_NPP", "MIN_NPP"]]
    return npp
...
def load_precipitation(path):
    precipitation = pd.read_csv(path, names=TEM_CLIMATE_COLUMNS)
    precipitation = precipitation[["LON", "LAT", "YEAR", "TOTAL", "AVE", "MAX", "MIN"]]
    precipitation.rename(columns={"TOTAL": "PRECIP_TOTAL", "AVE": "PRECIP_AVE", "MAX": "PRECIP_MAX", "MIN": "PRECIP_MIN"}, inplace=True)
...
precipitation_hist = load_precipitation("../historical/runs/climate_data/west_pr_gridmetcor_1750_2014.csv")


def load_temperature(path):
    temperature = pd.read_csv(path, names=TEM_CLIMATE_COLUMNS)
    temperature = temperature[["LON", "LAT", "YEAR", "TOTAL", "AVE", "MAX", "MIN"]]
    temperature.rename(columns={"TOTAL": "TEMP_TOTAL", "AVE": "TEMP_AVE", "MAX": "TEMP_MAX", "MIN": "TEMP_MIN"}, inplace=True)
    return temperature
...
temperature_hist = load_temperature("../historical/runs/climate_data/west_tair_gridmetcor_1750_2014.csv")


def load_moisture_stress(path):
    moisture_stress = pd.read_csv(path, names=MEAN_SUMMER_MOISTURE_STRESS_COLUMNS)
...
def load_elevation(path):
    elevation = pd.read_csv(path, names=["LON", "LAT", "VAR", "CAREA","ELEV", "REGION"])
...
def load_fire(path):
    ...
    fire = fire[["LON", "LAT","POTVEG", "YEAR", "MONTH", "FIRE_PROBABILITY","COUNT", "SEVERITY","THETA","STATUS","AREA_BURNED"]]
    fire = fire.groupby(["LON", "LAT", "POTVEG"]).agg({"FIRE_PROBABILITY": "mean", "SEVERITY": "mean", "COUNT": "sum", "THETA": "mean","STATUS": "mean","AREA_BURNED": "mean"})
...
def load_climate_limits(path):
    ...
    climate_limits = climate_limits.rename(columns=CLIMATE_LIMITS_COLUMNS)
    climate_limits = climate_limits[["POTVEG", "Tc", "Tw", "MIN_PREC", "MIN_AET/PET"]]
    return climate_limits

climate_limits_45 = load_climate_limits("../4.5/biogeography_module/pft_climate_limits.json")
climate_limits_85 = load_climate_limits("../8.5/biogeography_module/pft_climate_limits.json")
climate_limits_hist = load_climate_limits("../historical/biogeography_module/pft_climate_limits.json")



def combine_data(vegc, npp, precipitation, fire, temperature, moisture_stress, elevation, shift, climate_limits):
    ...
    merged_data = merged_data.merge(npp, on=["LON", "LAT","YEAR","POTVEG"], how="left")
    merged_data = merged_data.merge(precipitation, on=["LON", "LAT","YEAR",], how="left")
    merged_data = merged_data.merge(temperature, on=["LON", "LAT","YEAR",], how="left")
    merged_data = merged_data.merge(moisture_stress, on=["LON", "LAT","YEAR", "POTVEG"], how="left")
    ...
    merged_data[["FIRE_PROBABILITY", "COUNT", "SEVERITY", "THETA", "STATUS","AREA_BURNED"]] = merged_data[["FIRE_PROBABILITY", "COUNT", "SEVERITY", "THETA", "STATUS","AREA_BURNED"]].fillna(0)
...
# Usage
future_data_45 = combine_data(vegc_45,npp_45, precipitation_45, fire_45, temperature_45, moisture_stress_45, elevation_45, shift_45, climate_limits_45)
future_data_85 = combine_data(vegc_85,npp_85, precipitation_85, fire_85, temperature_85, moisture_stress_85, elevation_85, shift_85, climate_limits_85)
historical_data=combine_data(vegc_hist,npp_hist, precipitation_hist, fire_hist, temperature_hist, moisture_stress_hist, elevation_hist, shift_hist, climate_limits_hist)
...
def plot_elev_change(hist_data, fut_data_45, fut_data_85):
    ...
    all_data= pd.merge(historical, future_45, on='POTVEG', how='outer', suffixes=('_HIST', '_45'))
    all_data = pd.merge(all_data, future_85, on='POTVEG', how='outer').rename(columns={'ELEV':'ELEV_85', 'TEMP_AVE':'TEMP_AVE_85'})

    def missing(df,POTVEG,HIST_ELEV,HIST_TEMP):
        df.loc[(df['POTVEG'] == POTVEG) & (df['ELEV_HIST'].isnull()), 'ELEV_HIST'] = HIST_ELEV
        df.loc[(df['POTVEG'] == POTVEG) & (df['TEMP_AVE_HIST'].isnull()), 'TEMP_AVE_HIST'] = HIST_TEMP
        df.loc[(df['POTVEG'] == POTVEG) & (df['ELEV_45'].isnull()), 'ELEV_45'] = df['ELEV_HIST']
        df.loc[(df['POTVEG'] == POTVEG) & (df['TEMP_AVE_45'].isnull()), 'TEMP_AVE_45'] = HIST_TEMP+2
        df.loc[(df['POTVEG'] == POTVEG) & (df['ELEV_85'].isnull()), 'ELEV_85'] = df['ELEV_HIST']
        df.loc[(df['POTVEG'] == POTVEG) & (df['TEMP_AVE_85'].isnull()), 'TEMP_AVE_85'] = HIST_TEMP+3
    ...
    a= sns.lineplot(data=all_data, x="TEMP_AVE_HIST", y="ELEV_HIST", hue="POTVEG", palette=palette, 
                      style="POTVEG", marker="^", markersize=10, linewidth=2.5, linestyle='--')
    b = sns.lineplot(data=all_data, x="TEMP_AVE_45", y="ELEV_45", hue="POTVEG", palette=palette,
                        style="POTVEG", marker="o", markersize=10, linewidth=2.5, linestyle='--')
    c = sns.lineplot(data=all_data, x="TEMP_AVE_85", y="ELEV_85", hue="POTVEG", palette=palette,
                        style="POTVEG", marker="s", markersize=10, linewidth=2.5, linestyle='--')
    ...
    period_markers = {"^": "Historical\n(1984-2014)", "o": "RCP 4.5\n(2070-2100)", "s": "RCP 8.5\n(2070-2100)"}
    period_handles = [mlines.Line2D([], [], color='black', marker=marker, linestyle='None', fillstyle='none',
                                    markersize=8, label=label) for marker, label in period_markers.items()]

    # Create legend entries for POTVEG categories
    pft_handles = [mlines.Line2D([], [], color=color, marker='None', linestyle='-', linewidth=5,
                                label=potveg) for potveg, color in palette.items()]

    # Combine the handles
    all_handles = period_handles + pft_handles

    # Place the legend at the bottom center of the plot
    plt.legend(handles=all_handles, loc='upper center', bbox_to_anchor=(0.5, -0.10), ncol=4, frameon=False)
...
def plot_corr(hist_data, fut_data_45, fut_data_85):
    ...
    historical_subset = historical[['ELEV','TEMP_AVE', 'PRECIP_TOTAL', 'AET/PET','TOTAL_NPP']]
    
    future_subset_45 = future_45[['TEMP_AVE', 'PRECIP_TOTAL', 'AET/PET', 'THETA','AREA_BURNED', 'TOTAL_NPP', "TOTAL_VEGC"]]
    future_subset_45.rename(columns={"TEMP_AVE": "ave_tair", "PRECIP_TOTAL": "total_precip", "AET/PET": "aet/pet", "TOTAL_NPP": "npp","AREA_BURNED":"area_burned","THETA":"theta","TOTAL_VEGC":"vegc"}, inplace=True)
    future_subset_85 = future_85[['TEMP_AVE', 'PRECIP_TOTAL', 'AET/PET', 'THETA','AREA_BURNED','TOTAL_NPP', "TOTAL_VEGC"]]
    future_subset_85.rename(columns={"TEMP_AVE": "ave_tair", "PRECIP_TOTAL": "total_precip", "AET/PET": "aet/pet", "TOTAL_NPP": "total_npp","AREA_BURNED":"area_burned","THETA":"theta","TOTAL_VEGC":"vegc"}, inplace=True)
    ...
    sns.heatmap(future_corr_45, cmap='viridis', annot=True, vmin=-1, vmax=1, annot_kws={"size": 10}, ax=ax1)
    ax1.set_title("a) RCP 4.5")

    sns.heatmap(future_corr_85, cmap='viridis', annot=True, vmin=-1, vmax=1, annot_kws={"size": 10}, ax=ax2)
...
def plot_pca(fut_data,name):
    ...
    future_pca=future[['ELEV','TEMP_AVE', 'PRECIP_TOTAL', 'AET/PET','TOTAL_NPP', 'POTVEG','SEVERITY','AREA_BURNED']]

    # Extract the climatic variables from the combined data frame
    climatic_variables = future_pca[['TEMP_AVE', 'PRECIP_TOTAL', 'AET/PET','SEVERITY','AREA_BURNED']]
