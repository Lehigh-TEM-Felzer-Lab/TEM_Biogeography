...
fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(12, 6), sharex=True, sharey=True, subplot_kw={'projection': ccrs.Miller()})
...
for i, data in enumerate(data_list):
    ...
    mesh = ax[i].pcolormesh(lon, lat, grid_types, cmap=cmap, norm=norm, edgecolors='white', linewidth=0.5, transform=ccrs.PlateCarree(), shading='auto')
    ax[i].add_feature(cfeature.COASTLINE)
    ax[i].add_feature(cfeature.BORDERS, linestyle='-', alpha=.2)
    ax[i].add_feature(cfeature.STATES, linestyle='-', alpha=.2)
