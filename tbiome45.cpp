...
double Biome45::getVegSubarea(const int &tveg, const int &dtype, const int &carea)
{
    int mez;
    double sarea;
...
void Biome45::getvtype(const string &ecd)
{
    ...
    for (dv = 0; dv < NUMVEG; ++dv)
    {
        ...
        numtype[ez] = getXMLtvegArrayInt(infile, "communityECD", "numtype", vegtype);

        for (dtype = 0; dtype < NUMMSAC; ++dtype)
        {
            tempString.str("");
            tempString << "subtype" << (dtype + 1);
            subtype[ez][dtype] = getXMLtvegArrayInt(infile, "communityECD", tempString.str(), vegtype);

            tempString.str("");
            tempString << "pcttype" << (dtype + 1);
            pcttype[ez][dtype] = getXMLtvegArrayDouble(infile, "communityECD", tempString.str(), vegtype);
        }
