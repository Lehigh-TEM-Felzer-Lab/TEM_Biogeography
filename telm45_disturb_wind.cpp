...
void Telm45::atmswritemiss(ofstream fout[NUMATMS], const vector<string> &predname, const int &pdyr,
                           const int &natmspred, const double value)
{
    ...
    for (i = 0; i < natmspred; ++i)
    {
        ...
        atmspred.outdel(fout[i], col, row, predname[i], carea, atmstotyr[pdyr], atmspred.mon, contnent);
    }
};
...
void Telm45::atmswritepred(ofstream fout[NUMATMS], const int &pdyr, const vector<string> &predname,
                           const int &natmspred)
{
    ...
    for (i = 0; i < natmspred; ++i)
    {
        ...
        atmspred.outdel(fout[i], col, row, predname[i], carea, year, atmspred.mon, contnent);
    }
};
...
int Telm45::coregerr(ofstream &rflog1, const string &varname1, const float &col1, const float &row1,
                     const string &varname2, const float &col2, const float &row2)
{
...
int Telm45::equilibrateTEM(const double &ptol, const int &pichrt, ofstream ftempred[NUMTEM],
                           const vector<string> &tempredmap, const int &spinoutfg, int &year, ofstream &rflog1)
{
    ...
    while ((dyr < tem.runsize) && (tem.endeq < 2))
    {
...
#ifdef DEBUGT
            printf("\nclimate passed to tem in equilibrateTEM, month = %d, cdyear = %d, girr = %6.2lf, clds = %6.2lf, "
                   "nirr = %6.2lf, par = %5.2lf, prec = %8.21f, taird = %8.21f, co2 = %6.1lf, ws10 = %8.21f",
                   dm, cdyear, climate[clm.I_GIRR][dm][cdyear], climate[clm.I_CLDS][dm][cdyear],
                   climate[clm.I_NIRR][dm][cdyear], climate[clm.I_PAR][dm][cdyear], climate[clm.I_PREC][dm][cdyear],
                   climate[clm.I_TAIRD][dm][cdyear], climate[clm.I_CO2][dm][0], climate[clm.I_WS10][dm][cdyear]);
            // cout << " equilibrating, simulation year = " << dyr << endl;
#endif

#ifdef DEBUGT
            printf("\n before stepmonth, month = %d, dyr = %d, leafc = %5.2lf, soilc = %8.21f, soiln = %8.21f", dm, dyr,
                   tem.getY(0), tem.getY(6), tem.getY(13));
            // cout << " equilibrating, simulation year = " << dyr << endl;
#endif

            //      cout << "stepmonth = " << climate[clm.I_NIRR][dm][cdyear] << " " << cdyear << " " << dm << endl;
            tem.endeq = tem.stepmonth(dyr, dm, tem.intflag, ptol, pichrt, rflog1);

            //     cout << "dyr = " << dyr << endl;
#ifdef DEBUGT
            printf("\n after stepmonth, month = %d, dyr = %d, rphi = %4.2lf, rtair = %4.2lf, vegc = %8.2lf, gpp = "
                   "%5.21f, rh = %5.21f",
                   dm, dyr, tem.veg.getRPHI(), tem.veg.getRTAIR(), tem.veg.getVEGC(), tem.veg.getGPP(),
                   tem.microbe.getRH());
            // cout << " equilibrating, simulation year = " << dyr << endl;
#endif
        ...
        if (spinoutfg == 1)
        {
            temwritepred(ftempred, tempredmap, dyr, pichrt, ntempred, spinoutfg, 0);
        }
        ...
        //    cout << "vegequil = " << vegceq[(dyr%de_nyears)] << " " << vegceq[((dyr%de_nyears)+1)] << " " <<
        //    soilceq[(dyr%de_nyears)] << " " << soilceq[((dyr%de_nyears))+1] << " " << vegneq[(dyr%de_nyears)] << " "
        //    << vegneq[((dyr%de_nyears))+1] << " " << soilneq[(dyr%de_nyears)] << " " << soilneq[((dyr%de_nyears))+1]
        //    << " " << dyr << " " << dyr%de_nyears << " " << tem.veg.getVEGC() << " " << vegceq[((dyr+1)%de_nyears)] <<
        //    endl;
        ...
        if (dyr >= tem.strteq && 0 == tem.endeq)
        {
            ...
            tem.endeq = tem.testEquilibrium(dyr, de_nyears, vegceq[((dyr % de_nyears))], soilceq[((dyr % de_nyears))],
                                            vegneq[((dyr % de_nyears))], soilneq[((dyr % de_nyears))]);
        }
...
#ifdef DEBUGE
    cout << endl
         << " year, vegc, soilc, december prec at equilibrium " << dyr - 1 << " " << tem.veg.getVEGC() << " "
         << tem.soil.yrorgc << " " << climate[clm.I_PREC][11][cdyear] << endl;
#endif
...
void Telm45::getTEMCohortState(const int &pichrt)
{
    ...
    for (dm = 0; dm < CYCLE; ++dm)
    {
        tem.ag.setINITPROD1C(cohort[pichrt].initPROD1[dm].carbon, dm);

        tem.ag.setINITPROD1N(cohort[pichrt].initPROD1[dm].nitrogen, dm);
    }
...
void Telm45::outputTEMmonth(const int &pdm)
{
    ...
    if (0 == tem.ag.state)
    {
        ...
        output[tem.I_NATVEGC][pdm] = tem.getY(tem.I_LEAFC) + tem.getY(tem.I_SAPWOODC) + tem.getY(tem.I_HEARTWOODC) +
                                     tem.getY(tem.I_ROOTC) + tem.getY(tem.I_LABILEC);

        output[tem.I_CROPN][pdm] = ZERO;
        output[tem.I_NATVEGN][pdm] = tem.getY(tem.I_LEAFN) + tem.getY(tem.I_SAPWOODN) + tem.getY(tem.I_HEARTWOODN) +
                                     tem.getY(tem.I_ROOTN) + tem.getY(tem.I_LABILEN);
        ...
        output[tem.I_NATLTRN][pdm] =
            tem.getY(tem.I_LTRLN) + tem.getY(tem.I_LTRSN) + tem.getY(tem.I_LTRHN) + tem.getY(tem.I_LTRRN);
    }
    else
    {
        output[tem.I_CROPC][pdm] = tem.getY(tem.I_LEAFC) + tem.getY(tem.I_SAPWOODC) + tem.getY(tem.I_HEARTWOODC) +
                                   tem.getY(tem.I_ROOTC) + tem.getY(tem.I_LABILEC);
        output[tem.I_NATVEGC][pdm] = ZERO;

        output[tem.I_CROPN][pdm] = tem.getY(tem.I_LEAFN) + tem.getY(tem.I_SAPWOODN) + tem.getY(tem.I_HEARTWOODN) +
                                   tem.getY(tem.I_ROOTN) + tem.getY(tem.I_LABILEN);
        output[tem.I_NATVEGN][pdm] = ZERO;

        output[tem.I_CSTRN][pdm] =
            tem.getY(tem.I_LEAFN) + tem.getY(tem.I_SAPWOODN) + tem.getY(tem.I_HEARTWOODN) + tem.getY(tem.I_ROOTN);
        output[tem.I_NATSTRN][pdm] = ZERO;
        ...
        output[tem.I_AGLTRC][pdm] =
            tem.getY(tem.I_LTRLC) + tem.getY(tem.I_LTRSC) + tem.getY(tem.I_LTRHC) + tem.getY(tem.I_LTRRC);
...
void Telm45::readCohortState(ifstream &ifstate, const int &pichrt)
{
    int i;
    int dm;
    float dumflt;
...
void Telm45::setCohortTEMState(const ElmntCohort45 &firstchrt, ElmntCohort45 &targetchrt)
{
    int dm;
    int i;
...
int Telm45::setGIStopography(ofstream &rflog1, int &ftlerr, FILE *fstxt, FILE *felev)
{
    ...
    ftlerr = coregerr(rflog1, "Climate", col, row, "TEXTURE", fao.col, fao.row);
    ...
    ftlerr = coregerr(rflog1, "Climate", col, row, "ELEV", elv.col, elv.row);
...
void Telm45::setTEMequilState(ofstream &rflog1, const int &equil, const int &totsptime, const int &pichrt,
                              ofstream fout[NUMTEM], const vector<string> &predname, const int &spinoutfg)

{
    ...
    for (dm = 0; dm < CYCLE; ++dm)
    {
        ...
        cohort[pichrt].qc = temgisqc(cohort[pichrt].chrtarea, tem.soil.getPCTSAND(), tem.soil.getPCTSILT(),
                                     tem.soil.getPCTCLAY(), tem.veg.cmnt, tem.elev, tem.atms.getNIRR(),
                                     tem.atms.getPAR(), tem.atms.getTAIR(), tem.atms.getMXTAIR(), avetair[dyr],
                                     tem.atms.yrprec, tem.atms.getRAIN(), tem.atms.getSNOWFALL(), tem.atms.getCO2(),
                                     tem.atms.getAOT40(), tem.atms.getNDEP(), tem.atms.getTRANGE(), tem.atms.getVPR());
    ...
    if (cohort[pichrt].qc != ACCEPT)
    {
        ...
        if (cohort[pichrt].qc < 100
            //        && tem.veg.cmnt > 1
            && tem.veg.cmnt > 0 && (mxtair[dyr] < -1.0 || yrprec[dyr] <= ZERO))
        {
        ...
        setTEMmiss(dyr, equil, totsptime, pichrt);
    }
    else // "cohort[pichrt].qc == ACCEPT"
    {
        ...
        ************************************************************* */
        ...
        {
            tem.soil.xtext(tem.ag.cmnt, tem.soil.getPCTSILT(), tem.soil.getPCTCLAY());
        }
        ...
        for (dm = 0; dm < CYCLE; ++dm)
        {
            if (tem.ag.state == 0)
            {
                tem.veg.phenology(tem.veg.cmnt, climate[clm.I_TAIR][dm][dyr], climate[clm.I_TAIRD][dm][dyr],
                                  climate[clm.I_TAIRN][dm][dyr], 1.0, 1.0, 0.0);
            }
            else
            {
                tem.veg.phenology(tem.ag.cmnt, climate[clm.I_TAIR][dm][dyr], climate[clm.I_TAIRD][dm][dyr],
                                  climate[clm.I_TAIRN][dm][dyr], 1.0, 1.0, 0.0);
            }
        ...
        if (0 == equil)
        {
            ...
            cohort[pichrt].tqc = transqc(tem.maxyears, tem.totyr,
                                         //                                    tem.veg.getVEGC() );
...
void Telm45::setTEMmiss(const int &pdyr, const int &equil, const int &totsptime, const int &pichrt)
{
    int dm;
    int i;
...
int Telm45::temgisqc(const long &subarea, const double &pctsand, const double &pctsilt, const double &pctclay,
                     const int &cmnt, const double &elev, const double &nirr, const double &par, const double &tair,
                     const double &mxtair, const double &avtair, const double &yrprec, const double &rain,
                     const double &snowfall, const double &co2, const double &aot40, const double &ndep,
                     const double &trange, const double &vpd)
...
void Telm45::temwritepred(ofstream fout[NUMTEM], const vector<string> &predname, const int &pdyr, const int &pichrt,
                          const int &ntempred, const int &spinoutfg, const int &equilfg)
{
    ...
    for (i = 0; i < ntempred; ++i)
    {
        ...
        else if (predname.at(i) == tem.predstr.at(tem.I_SRUN))
        {
            for (dm = 0; dm < CYCLE; ++dm)
            {
                ...
                //        (1-tem.rfrac[dm])*((tem.wevapd[dm]+tem.wevapn[dm])/2);
                tempred.mon[dm] = output[tem.I_SRUN][dm];
            }
        }
        ...
        else if (predname.at(i) == tem.predstr.at(tem.I_WYLD))
        {
            for (dm = 0; dm < CYCLE; ++dm)
            {
                ...
                //        cout << "wyld = " << dm << " " << output[tem.I_WYLD][dm] << " " << tem.wevapd[dm] << " " <<
                //        tem.wevapn[dm] << endl;
            }
        }
        ...
        if (equilfg == 1)
        {
            ...
            {
                tempred.poutdel(fout[i], col, row, predname.at(i), (pichrt + 1), cohort[pichrt].standage,
                                tem.veg.getPOTVEG(), tem.veg.getCURRENTVEG(), tem.veg.getSUBTYPE(), tem.veg.cmnt,
                                (PROP2PCT * tem.soil.getPSIPLUSC()), tem.qualcon[pdyr], carea, cohort[pichrt].chrtarea,
                                ttotyr[pdyr], tempred.mon, region);
            }
            else
            {
                tempred.outdel(fout[i], col, row, predname.at(i), (pichrt + 1), cohort[pichrt].standage,
                               tem.veg.getPOTVEG(), tem.veg.getCURRENTVEG(), tem.veg.getSUBTYPE(), tem.veg.cmnt,
                               (PROP2PCT * tem.soil.getPSIPLUSC()), tem.qualcon[pdyr], carea, cohort[pichrt].chrtarea,
                               ttotyr[pdyr], tempred.mon, region);
            }
...
int Telm45::transqc(int &maxyears, int &totyr, double plantc[CYCLE])
{
...
void Telm45::updateTEMmonth(const int &equil, const int &totsptime, const int &pdyr, const int &pdm, const int &pichrt,
                            ofstream &rflog1)
{
    ...
    if (ACCEPT == cohort[pichrt].qc && ACCEPT == cohort[pichrt].tqc)
    {
        ...
        //
        //     tem.ag.fert1950flag = 0;
        // cout << "totyr = " << tem.totyr << " " << tem.ag.fert1950flag << " " << pdyr << " " << pdm << " " << pichrt
        // << endl;
        ...
        wrtyr = tem.monthlyTransient(pdyr, pdm, tem.tol, pichrt, rflog1);
    ...
    else
    {
        if ((CYCLE - 1) == pdm)
        {
            ...
            setTEMmiss(pdyr, equil, totsptime, pichrt);
...
void Telm45::writeCohortState(ofstream &ofstate, const int &pichrt)
{
    int i;
    int dm;
