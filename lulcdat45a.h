...
class Lulcdat45
{
  ...
  public:
    ...
    ************************************************************* */
    ...
    void out(ofstream &ofile, const float &col, const float &row, const string &varname, const int &year,
             const int &icohort, const int &isrccohort, const int &standage, const long &chrtarea, const int &potveg,
             const int &currentveg, const int &subtype, const int &agstate, const int &agprevstate, const int &tillflag,
             const int &fertflag, const int &irrgflag, const int &disturbflag, const int &disturbmonth, const int &FRI,
             const double &slashpar, const double &vconvert, const double &prod10par, const double &prod100par,
             const double &vrespar, const double &sconvert, const string &region);

    void outdel(ofstream &ofile, const float &col, const float &row, const string &varname, const int &year,
                const int &icohort, const int &isrccohort, const int &standage, const long &chrtarea, const int &potveg,
                const int &currentveg, const int &subtype, const int &agstate, const int &agprevstate,
                const int &tillflag, const int &fertflag, const int &irrgflag, const int &disturbflag,
                const int &disturbmonth, const int &FRI, const double &slashpar, const double &vconvert,
                const double &prod10par, const double &prod100par, const double &vrespar, const double &sconvert,
                const string &region);
