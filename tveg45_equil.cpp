...
void Tveg45::aerodynamics(const int &pdcmnt, const double &windspeed)
{
    /**
    ...
    */
    ...
    if (ua < 0.0)
    {
        cout << " fatal error! ua < 0 " << ua << " " << za << " " << zh << " " << zd << " " << zo << " " << cabove
             << " " << labile.carbon << endl;
    }
    ...
    //  cout << "diag = " << pen.getR_AS() << " " << zh << " " << n_eddy << " " << zd << " " << zo << " " << ustar <<
    //  endl;
...
/* *************************************************************
************************************************************* */
void Tveg45::allocate(const int &pdcmnt, const int &pndays, const int &nfeed, const int &pdm, const double &gdd,
                      const int &agstate)
{
    /**
    ...
    */
    ...
    //  cout << "alloc diag = " << mb_l << " " << mb_s << " " << mb_r << " " << mcm_l << " " << mcm_s << " " << mcm_r <<
    //  " " << mcc_l << " " << mcc_s << " " << mcc_r << " " << fleafc << " " << fsapwoodc << " " << frootc << endl;
    ...
    if ((leaf.rm + sapwood.rm + root.rm + labile.rm) <= labile.carbon)
    {
        ...
        if (labile.carbon > 0.67 * (fsaplive[pdcmnt] * sapwood.carbon + root.carbon + leaf.carbon))
        {
            ...
            norm = 1.0 / (pleaflc / cnleaf[pdcmnt] + pseedc / cnseed[pdcmnt] + psapwoodlc / cnsapwood[pdcmnt] +
                          prootlc / cnroot[pdcmnt]);
            ...
            leaf.allocn += pleafln * ((extra / labile.carbon) * labile.nitrogen) * (12.0 * pndays / 365.0);
            seed.allocn += pseedn * ((extra / labile.carbon) * labile.nitrogen) * (12.0 * pndays / 365.0);
            sapwood.allocn += psapwoodln * ((extra / labile.carbon) * labile.nitrogen) * (12.0 * pndays / 365.0);
            root.allocn += prootln * ((extra / labile.carbon) * labile.nitrogen) * (12.0 * pndays / 365.0);
    ...
    cnprod =
        leaf.allocc + sapwood.allocc + root.allocc + seed.allocc + leaf.rg + sapwood.rg + root.rg + seed.rg + labile.rm;
    ...
    // cout << "diag = " << mb_s <<  " " << fsapwoodc << " " << cnsapwood[pdcmnt] << " " << tausapwood[pdcmnt] << " " <<
    // labile.carbon << " " << extra << " " << pndays << " " << sapwood.rm << " " << sapwood.allocc << endl; cout <<
    // "diag = " << fsaplive[pdcmnt] << " " << sapwood.carbon << " " << root.carbon << " " << leaf.carbon << " " <<
    // psapwoodlc << " " << cnsapwood[pdcmnt] << endl;
};
/* *************************************************************
************************************************************* */
...
void Tveg45::deltafo3(const int &pdcmnt, const double &d40)
...
void Tveg45::getecd(const string &ecd)
{
    ...
    */
    ...
    for (dcmnt = 1; dcmnt < MAXCMNT; ++dcmnt)
    {
        ...
        kc[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "kc", comtype);

        ki[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "ki", comtype);

        tmin[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "tmin", comtype);

        toptmin[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "toptmin", comtype);

        toptmax[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "toptmax", comtype);

        tmax[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "tmax", comtype);

        raalpha[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "raalpha", comtype);

        rabeta[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "rabeta", comtype);

        ragamma[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "ragamma", comtype);

        raqref[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "raqref", comtype);

        ratref[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "ratref", comtype);

        kn1[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "kn1", comtype);

        sla[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "sla", comtype);

        href[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "href", comtype);

        krnup[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "krnup", comtype);

        kallom[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "kallom", comtype);

        cnleaf[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "cnleaf", comtype);

        cnsapwood[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "cnsapwood", comtype);

        cnheartwood[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "cnheartwood", comtype);

        cnroot[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "cnroot", comtype);

        cnseed[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "cnseed", comtype);

        pseedlc[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "pseedlc", comtype);

        fsaplive[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "fsaplive", comtype);

        phen[comtype] = getXMLcmntArrayInt(infile, "vegECD", "phen", comtype);

        nnf[comtype] = getXMLcmntArrayDouble(infile, "vegECD", "nnf", comtype);
...
void Tveg45::gppxclm(const int &pdcmnt, const int &pndays, const double &co2, const double &par, const double &vpr,
                     const double &vpd, const double &pdayl, const double &kext, const double &rhmcrb,
                     const double &prec, const double &vegc)
{
    /**
    ...
    */
    ...
    if (lai > 0.01 && fp > ZERO)
    {
        ci = setCI(pdcmnt, pndays, fp, fd, co2, rhmcrb, pdayl);
    }
    else
    {
        ci = co2;
    ...
    //  cout << "veggpp " << veggpp << " " << cmax << " " << cmaxgcmo << " " <<  frdl << " " << fco2 << " " << temp << "
    //  " << fozone << " " << fh2o << " " << lai << endl; cout << "frdl " << frdl << " " << kext << " " << kiw << " " <<
    //  parw << " " <<  lai << endl;
    ...
    //  cout << "dgppdleafc = " << dgppdleafc << " " << cmaxgcmo << " " << frad << " " << frdl << " " << fco2 << " " <<
    //  temp << " " << fozone << " " << fh2o << " " << pen.getDFWS_DLEAFC() << endl;
    ...
    {
        cnavg = 1.0 / (yrpleaf / cnleaf[pdcmnt] + yrpsapwood / cnsapwood[pdcmnt] + yrproot / cnroot[pdcmnt] +
                       yrpseed / cnseed[pdcmnt]);
    }
...
void Tveg45::litterresp(const int &pdcmnt, const int &pndays)
{
    /**
...
void Tveg45::nupxclm(const int &pdcmnt, const double &nmax_grow, const double &soilh2o, const double &availn,
                     const double &rofT, const double &ksoil, const double &fozone, const double &rootmass)
{
    /**
...
/* *************************************************************
************************************************************* */
void Tveg45::petsw(const int &pdcmnt, const int &pdm, const int &pndays, const double &pdayl, const double &taird,
                   const double &tairn, const double &co2, const double &nirrn, const double &lwoutd,
                   const double &lwoutn, const double &vpr, const double &vpdd, const double &vpdn,
                   const double &snowcover, const double &precip, const double &soilmmmo, const double &avlh2o,
                   const double &elev)

{
    ...
    */
    ...
    double npet, dpet, npet_pt, dpet_pt, npet_act, dpet_act;
    double dnirr, ngc;
    double rcanmm; // canopy respiration, in mmol m^2 s^-1

    rcanmm = (leaf.rm + leaf.rg) / (12.0e-3 * 24.0 * 3600.0 * pndays);

    gc = pen.gcpet(pdcmnt, pdm, lai, gpp, rcanmm, vpr, vpdd, co2, pdayl);

    ngc = pen.gcpet(pdcmnt, pdm, lai, 0.0, 0.0, vpr, vpdn, co2, 24.0 - pdayl);
    ...
    dpet = pen.shuttlewallace(pdcmnt, dnirr, lwoutd, lai, taird, vpdd, pdm, gc, snowcover, pdayl * 3600.0, precip,
                              esoilmmmo, elev);
    ...
    npet = pen.shuttlewallace(pdcmnt, 0.0, lwoutn, lai, tairn, vpdn, pdm, ngc, snowcover, (24.0 - pdayl) * 3600.0,
                              precip, esoilmmmo, elev);
    ...
    dpet_pt = pen.potevap(pdcmnt, dnirr, lwoutd, lai, taird, vpdd, pdm, gc, snowcover, pdayl * 3600.0, precip,
                          esoilmmmo, elev);
    npet_pt = pen.potevap(pdcmnt, 0.0, lwoutn, lai, tairn, vpdn, pdm, ngc, snowcover, (24.0 - pdayl) * 3600.0, precip,
                          esoilmmmo, elev);

    potevap = (dpet_pt + npet_pt);

    dpet_act = pen.actevap(pdcmnt, dnirr, lwoutd, lai, taird, vpdd, pdm, gc, snowcover, pdayl * 3600.0, precip,
                           esoilmmmo, elev);
    npet_act = pen.actevap(pdcmnt, 0.0, lwoutn, lai, tairn, vpdn, pdm, ngc, snowcover, (24.0 - pdayl) * 3600.0, precip,
                           esoilmmmo, elev);

    actevap = (dpet_act + npet_act);
};
...
/* *************************************************************
************************************************************* */
void Tveg45::phenology(const int &pdcmnt, const double &tair, const double &ptaird, const double &ptairn,
...
void Tveg45::resetEcds(const int &pcmnt, const double &psiplusc)
{
    /**
...
void Tveg45::resetNEWTOPT(const int &pcmnt, const double &tair, const double &prtairphi, const double &prphi)
{
    /**
...
void Tveg45::rxTLaRS(const int &pdcmnt, const double &ptair, const double &ptref)
{
    /**
    ...
    */
    ...
    // cout << "rmt = " << rmt << " " << ptair << " " << ptref << " " << topt << " " << " " << raqref[pdcmnt] << " " <<
    // " " << raalpha[pdcmnt] << " " << rabeta[pdcmnt] << " " << ragamma[pdcmnt] << endl;
};
...
double Tveg45::setCI(const int &pdcmnt, const int &pndays, const double &fp, const double &fd, const double &ca,
                     const double &rhmicrobe, const double &pdayl)
{
    /**
...
/* *************************************************************
************************************************************* */
void Tveg45::updateDynamics(const int &pdcmnt, const double &ninput, const double &availn, const int &nfeed,
                            ...
                            double &agfertn, const double &soninp)
{
    ...
    */
    ...
    if (1 == nfeed)
    {
        ...
        if (rdemandn > ZERO && rdemandc > ZERO && rlabilen > ZERO && rlabilec > ZERO && labile.nitrogen > ZERO &&
            labile.carbon > ZERO && cnprod > ZERO)
        {
        ...
        //    cout << "diag = " << gpp << " " << ingpp << " " << fgppdownreg << " " << rdemandc << " " << rdemandn << "
        //    " << rlabilec << " " << rlabilen <<  endl; exit(-1);
    }
};
