...
void Humnact45::conversion(const int &pdcmnt, const double &slashc, const double &slashn, const double &vegc,
                           const double &vstrn, const double &vston, const double &soilc, const double &soiln,
                           const double &cwdloss, const int &time)
{
    ...
    vconvrtflx.nitrogen =
        ((1.0 - nvretconv[pdcmnt]) * vconvert * (vstrn + vston + cwdloss * standdead.nitrogen)) / (double)time;
...
void Humnact45::createWoodProducts(const int &pdyr, const double &stemc, const double &stemn)
{
    int i;
...
void Humnact45::decayProducts(void)
{
    ...
    //  cout << "decay = " << PROD1decay.carbon << " " << PROD10decay.carbon << " " << PROD100decay.carbon << " " <<
    //  animalresp << " " << TOTPRODdecay.carbon << endl;
...
void Humnact45::frostDamage(const double &vegc, const double &vegn)
{
    frostflag = 1;
...
void Humnact45::getecd(const string &ecd)
{
    ...
    for (dcmnt = 1; dcmnt < MAXCMNT; ++dcmnt)
    {
        ...
        nvretconv[comtype] = getXMLcmntArrayDouble(infile, "agECD", "nvretconv", comtype);

        nsretconv[comtype] = getXMLcmntArrayDouble(infile, "agECD", "nsretconv", comtype);

        tillfactor[comtype] = getXMLcmntArrayDouble(infile, "agECD", "tillfactor", comtype);

        harvstC[comtype] = getXMLcmntArrayDouble(infile, "agECD", "harvstC", comtype);

        harvstN[comtype] = getXMLcmntArrayDouble(infile, "agECD", "harvstN", comtype);

        residueC[comtype] = getXMLcmntArrayDouble(infile, "agECD", "residueC", comtype);

        residueN[comtype] = getXMLcmntArrayDouble(infile, "agECD", "residueN", comtype);

        cropseedC[comtype] = getXMLcmntArrayDouble(infile, "agECD", "cropseedC", comtype);

        cropseedSTON[comtype] = getXMLcmntArrayDouble(infile, "agECD", "cropseedSTON", comtype);

        GDDMIN[comtype] = getXMLcmntArrayDouble(infile, "agECD", "gddmin", comtype);

        GDDSEED[comtype] = getXMLcmntArrayDouble(infile, "agECD", "gddseed", comtype);

        GDDHARVST[comtype] = getXMLcmntArrayDouble(infile, "agECD", "gddharvst", comtype);

        tkill[comtype] = getXMLcmntArrayDouble(infile, "agECD", "tkill", comtype);

        isPerennial[comtype] = getXMLcmntArrayInt(infile, "agECD", "isPerennial", comtype);
...
/* *************************************************************
************************************************************* */
void Humnact45::grazing(const double &leafc, const double &sapwoodc, const double &heartwoodc, const double &labilec,
                        const double &leafn, const double &sapwoodn, const double &heartwoodn, const double &labilen,
                        const double &rootc)
{
...
void Humnact45::harvest(const int &pdm, const double &seedc, const double &seedn, const double &vegc,
                        const double &vegn)
{
    ...
    // cout << "stubble terms = " << stubble.carbon << " " << initresidueC << " " << vegc << " " << seedc << " " <<
    // cropprod.carbon << endl;
    //   fert1950flag = 1;
};
...
/* *************************************************************
************************************************************* */
void Humnact45::standingdead(const int &pdcmnt, const double &slashc, const double &slashn, const double &vegc,
...
void Humnact45::updatestanddead(const int &pdyr, const double &rhmoist, const double &dq10, const double &cnltr)
{
    ...
    if (standdead.carbon > 0)
    {
        ...
        //  cout << "standdead = " << slash.carbon << " " << standdead.carbon << " " << slash.nitrogen << " " <<
        //  standdead.nitrogen << endl;
    ...
    //  cout << "diag = " << immbadd << " " << c_add << " " << n_add << " " << cnltr << " " << rhmoist << " " << dq10 <<
    //  endl;
    ...
    if (1 == state)
    {
        for (kdm = 0; kdm < CYCLE; ++kdm)
        {
            cropResidueFlux.carbon +=
                initPROD1[kdm].carbon * (1.000000 - harvstC[cmnt]) / harvstC[cmnt] * residueC[cmnt] / (double)CYCLE;

            cropResidueFlux.nitrogen +=
                initPROD1[kdm].nitrogen * (1.000000 - harvstN[cmnt]) / harvstN[cmnt] * residueN[cmnt] / (double)CYCLE;
        }
    }
};
/* *************************************************************
