...
void Ttem45::cropDynamics(const int &pdm, const int &pdyr, const double &nmax_grow, double pstate[])
{
    ...
    // cout << "diag = " << ag.fert1950flag << " " << ag.state << " " << pdm << " " << initFlag  << " " << ag.fertflag
    // << endl;
    ...
    // cout << "eet terms = " << atms.getNDAYS(pdm) << " " << atms.getDAYL() << " " << atms.getTAIRD() << " " <<
    // atms.getTAIRN() << " " << atms.getCO2() << " " << atms.getNIRRN() << " " << atms.getLWOUTD() << " " <<
    // atms.getLWOUTN() << " " << atms.getVPR() << " " << atms.getVPDN() << " " << soil.getSNOWPACK() << " " <<
    // atms.getPREC() << " " << veg.getESOILMMMO() << " " << soil.getAVLH2O() << " " << elev << endl;
    ...
    // cout << " water terms = " << atms.getRAIN() << " " << pstate[I_RGRW] << " " << soil.getRPERC() << " " <<
    // soil.getRRUN() << " " << soil.getAVLH2O() << " " << soil.getAWCAPMM() << " " << y[I_SM] << " " <<
    // soil.getWILTPT() << endl;
...
void Ttem45::delta(const int &pdm, const int &pdyr, const double &nmax_grow, double pstate[], double pdstate[])
{
    ...
    // if(veg.cmnt == 15) {cout << "pdavln = " << pdstate[I_AVLN] << " " << soil.getNINPUT() << " " <<
    // microbe.getNETNMIN() << " " <<  ag.getVOLAN() << " " << veg.getNUPTAKE() << " " << soil.getNLOST() << " " <<
    // pstate[I_AVLN] << " " << soil.getRRUN() << " " << soil.getSRUN() << " " << pstate[I_SM] << endl;}
    ...
    // if(veg.cmnt == 15) {cout << "delta sm = " << pdstate[I_SM] << " " << pstate[I_SM] << " " << soil.getEET() << " "
    // << atms.getRAIN() << " " << soil.getRPERC() << " " << soil.getSPERC() << endl;}
...
void Ttem45::massbal(void)
{
    ...
    if ((y[I_SM] - prevy[I_SM]) !=
        (soil.getSNOWINF() + atms.getRAIN() + y[I_AGIRRIG] - y[I_RPERC] - y[I_EET] - y[I_SPERC]))
    {
    ...
    if (y[I_LABILEC] - prevy[I_LABILEC] !=
        y[I_GPP] - y[I_ALLOCLC] - y[I_ALLOCSC] - y[I_ALLOCRC] - -y[I_ALLOCSEEDC] - y[I_RVGRW])
    {
        y[I_RVGRW] =
            y[I_GPP] - y[I_ALLOCLC] - y[I_ALLOCSC] - y[I_ALLOCRC] - y[I_ALLOCSEEDC] - y[I_LABILEC] + prevy[I_LABILEC];
    }
    ...
    {
        y[I_RH] = y[I_LTRLC] + y[I_LTRSC] + y[I_LTRHC] + y[I_LTRRC] + y[I_LTRSEEDC] + ag.getSLASHC() -
                  ag.getSCONVRTFLXC() - y[I_DOCPROD] - y[I_SOLC] + prevy[I_SOLC];
    }
    ...
    if (y[I_LABILEN] - prevy[I_LABILEN] != y[I_VNUP] + y[I_NRESORBL] + y[I_NRESORBS] + y[I_NRESORBR] +
                                               y[I_NRESORBSEED] + y[I_NFIXS] - y[I_ALLOCLN] - y[I_ALLOCSN] -
                                               y[I_ALLOCRN] - y[I_ALLOCSEEDN])
    {
        y[I_NRESORBL] = y[I_ALLOCLN] + y[I_ALLOCSN] + y[I_ALLOCRN] + y[I_ALLOCSEEDN] + y[I_NFIXS] - y[I_VNUP] +
                        y[I_LABILEN] - prevy[I_LABILEN] - y[I_NRESORBS] - y[I_NRESORBR] - y[I_NRESORBSEED];
    }
    ...
    if (y[I_SOLN] - prevy[I_SOLN] != y[I_LTRLN] + y[I_LTRSN] + y[I_LTRHN] + y[I_LTRRN] + y[I_LTRSEEDN] +
                                         ag.getSLASHN() + y[I_NFIXN] - y[I_DONPROD] - ag.getSCONVRTFLXN() -
                                         ag.getNSRETENT() - y[I_NMIN])
    {
        y[I_NMIN] = y[I_LTRLN] + y[I_LTRSN] + y[I_LTRHN] + y[I_LTRRN] + y[I_LTRSEEDN] + ag.getSLASHN() + y[I_NFIXN] -
                    y[I_DONPROD] - ag.getSCONVRTFLXN() - ag.getNSRETENT() - y[I_SOLN] + prevy[I_SOLN];
    }
...
#else
    if (y[I_SOLN] - prevy[I_SOLN] != y[I_LTRLN] + y[I_LTRSN] + y[I_LTRHN] + y[I_LTRRN] + y[I_LTRSEEDN] +
                                         ag.getSLASHN() - ag.getSCONVRTFLXN() - ag.getNSRETENT() - y[I_NMIN])
    {
        y[I_NMIN] = y[I_LTRLN] + y[I_LTRSN] + y[I_LTRHN] + y[I_LTRRN] + y[I_LTRSEEDN] + ag.getSLASHN() -
                    ag.getSCONVRTFLXN() - ag.getNSRETENT() - y[I_SOLN] + prevy[I_SOLN];
    }
                             ...
                             const float &col, const float &row)
{
    ...
    {
        y[I_VSM] = soil.updateRootZ(veg.cmnt, y[I_SM], y[I_ROOTC]);

        veg.resetEcds(veg.cmnt, soil.getPSIPLUSC());
    }
    else
    {
        y[I_VSM] = soil.updateRootZ(ag.cmnt, y[I_SM], y[I_ROOTC]);
    ...
    stepmonth(pdyr, pdm, intflag, ptol, ichrt, rflog1, col, row);
    ...
    veg.pen.setR_SS(
        exp(8.206 -
            4.205 * sqrt(surf_wetness * prob))); // using formulation from SiB2
                                                 //  }
                                                 //  veg.pen.setR_SS((11.489*exp(-11.67*soil.getVSM()))* exp(8.206
                                                 //  - 4.205*sqrt(surf_wetness*prob)) ); // using formulation from SiB2
                                                 //  veg.pen.setR_SS((78.935*exp(-20.7*soil.getVSM()))* exp(8.206
                                                 //  - 4.205*sqrt(surf_wetness*prob)) ); // using formulation from SiB2
                                                 //  veg.pen.setR_SS((-4.64*log(soil.getVSM())-6.5404)* exp(8.206
                                                 //  - 4.205*sqrt(surf_wetness*prob)) ); // using formulation from SiB2
#endif
...
int Ttem45::stepmonth(const int &pdyr, const int &pdm, int &intflag, const double &ptol, const int &ichrt,
                      ofstream &rflog1, const float &col, const float &row)
{
    ...
    // fout << "Entering Stepmonth " << veg.cmnt << " " << pdyr << " " << pdm << " " << y[I_GPP] << endl;
    ...
    RH = atms.getVPR() / (((atms.getVPDD() + atms.getVPDN()) / 2.0 + atms.getVPR())) * 100.0;
    theta = (y[I_SM] - soil.getWILTPT()) / soil.getAWCAPMM(); // soil moisture relative to that at saturation
    theta_e = 0.30;                                           // the moisture of extinction

    // Moisture Index Output AET/PET (Monthly Values)

    /*
    Actual Evapotranspiration:

    It is the quantity of water that is actually removed from a surface due to the processes of evaporation and
    transpiration

    Potential Evapotranspiration:

    It is a measure of the ability of the atmosphere to remove water from the surface through the processes of
    evaporation and transpiration, assuming no control over the water supply.

    The relation between actual evapotranspiration and potential evapotranspiration with different soil moisture
    conditions is as follows:

    If the water supply to the plant is adequate, soil moisture will be at field capacity then, the ratio of AET
    to PET = 1.
    ...
    Hence, If the moisture content in the soil is at the field capacity of the soil, the ratio of Actual
    Evapotranspiration (AET) to the potential Evapotranspiration (PET) is equal to 1.

    # Additional Information

    Field Capacity (FC):

    Field capacity is the maximum quantity of water that the soil at the field simply drains away.

    Permanent Wilting point (PWT):

    A permanent wilting point is the moisture content of soil at which the moisture is no longer available in sufficient
    */
    ...
    // Fire occurrence - Adapted from (Melton & Arora, 2016), (Lawrence et al., 2018) & (Li et al., 2012)
    /*
        ...
        if ((initFlag == 1) && (pdyr >= 0) && (pdm >= 4) && (pdm <= 10) &&
            (firecount[ichrt] == 0 && veg.getVEGC() > 1050.0) && (RH < 70.0) && (theta < theta_e))
    ...
    */
    ...
    if (disturbflag == 1 && pdm == (disturbmonth - 1)) // agriculture
    {
        ...
        ag.conversion(veg.cmnt,
                      ...
                      y[I_LEAFN] / ag.getVCONVERT() + y[I_SAPWOODN] + y[I_HEARTWOODN], y[I_LABILEN], y[I_SOLC],
                      y[I_SOLN], cwdloss, CYCLE);
        ...
        ag.standingdead(veg.cmnt, (y[I_ROOTC]), (y[I_ROOTN]), 0.0, 0.0, cwdloss);
    ...
    else if (disturbflag == 2 && pdm == (disturbmonth - 1)) // timber harvest clearcut
    {
        ...
        // Original Approach
        //
        ag.conversion(veg.cmnt, timber * (y[I_LEAFC] + y[I_ROOTC] + y[I_LABILEC]), timber * (y[I_LEAFN] + y[I_ROOTN]),
                      timber * (y[I_SAPWOODC] + y[I_HEARTWOODC]), timber * (y[I_SAPWOODN] + y[I_HEARTWOODN]),
                      timber * y[I_LABILEN], timber * y[I_SOLC], timber * y[I_SOLN], cwdloss, CYCLE);
    ...
    else if ((disturbflag == 3) || (fireoccur == 1)) // fire, mid intensity
    {
        // cout << " Fire Event: " << firecount[ichrt] << " Year: " << pdyr << " Month: " << pdm << " Cohort: " << ichrt
        // << endl;
        //  ADDED PCP
        //  ag.setVCONVERT(1.0);
        ...
        ag.conversion(veg.cmnt, dwood * y[I_ROOTC], dwood * y[I_ROOTN],
                      ...
                      dwood * y[I_LABILEN], y[I_SOLC], y[I_SOLN], cwdloss, 1);
        //
        ////  overwrite the slash with same as above but add to standing dead
        ////
        ag.standingdead(veg.cmnt, dwood * y[I_ROOTC], dwood * y[I_ROOTN],
                        ...
                        // + dleaf*y[I_LEAFN]),
                        (1.0 - ag.getVCONVERT()) * (dwood * (y[I_SAPWOODN] + y[I_HEARTWOODN] + y[I_LABILEN])), cwdloss);
    ...
    else if (disturbflag == 4 && pdm == (disturbmonth - 1)) // hurricane-strength storm
    {
        ...
        ag.standingdead(veg.cmnt,
                        ...
                        0.99 * y[I_LEAFC] + 0.99 * (y[I_ROOTC]), 0.99 * y[I_LEAFN] + 0.99 * (y[I_ROOTN]),
                        0.99 * (y[I_SAPWOODC] + y[I_HEARTWOODC] + y[I_LABILEC]),
                        0.99 * (y[I_SAPWOODN] + y[I_HEARTWOODN] + y[I_LABILEN]), cwdloss);

        /*
                0.2*y[I_LEAFC]+0.11*(y[I_ROOTC]),
                0.2*y[I_LEAFN]+0.11*(y[I_ROOTN]),
                0.11*(y[I_SAPWOODC]+y[I_HEARTWOODC]+y[I_LABILEC]),
                0.11*(y[I_SAPWOODN]+y[I_HEARTWOODN]+y[I_LABILEN]),cwdloss );
        ...
        ag.state == 0)
    {
        ...
        if (tempret_storm != 0 && ((int)rand() % (tempret_storm - 1)) == 1)
        {
            ...
            //
            ag.standingdead(veg.cmnt, 0.07 * y[I_LEAFC] + 0.04 * (y[I_ROOTC]), 0.07 * y[I_LEAFN] + 0.04 * (y[I_ROOTN]),
                            (0.04 * (y[I_SAPWOODC] + y[I_HEARTWOODC] + y[I_LABILEC])),
                            (0.04 * (y[I_SAPWOODN] + y[I_HEARTWOODN] + y[I_LABILEN])), cwdloss);
        ...
        if (tempret_hurr != 0 && ((int)rand() % (tempret_hurr - 1)) == 1)
        {
            ...
            ag.standingdead(veg.cmnt, 0.2 * y[I_LEAFC] + 0.11 * (y[I_ROOTC]), 0.2 * y[I_LEAFN] + 0.11 * (y[I_ROOTN]),
                            0.11 * (y[I_SAPWOODC] + y[I_HEARTWOODC] + y[I_LABILEC]),
                            0.11 * (y[I_SAPWOODN] + y[I_HEARTWOODN] + y[I_LABILEN]), cwdloss);
    ...
    if ((initFlag == 1) && pdm == 7 && ag.state == 0 && pdyr > 1 && stormoccur == 0)
    {
        ...
        if ((dleaf != 0 || dwood != 0))
        {
            ...
            firecount[ichrt]++; // Increment fire count
            //  cout << "fire = " << pdyr << " " << veg.cmnt << " " << dwood << " " << dleaf << " " << lowi << " "
            //  << mixi << " " << repi << " " << firemag << " " << vegmax << " " << replace << " " << veg.getVEGC()
            //  << " " << rhmoist << " " << dq10 << endl;
    ...
    rhmoist =
        microbe.setRHMOIST(veg.cmnt, soil.getPCTFLDCAP(), soil.getPCTWILTPT(), soil.getPCTPOR(), y[I_VSM], moistlim);
    ...
    if (0 == pdyr && 0 == pdm)
    {
        ...
        y[I_VSM] = soil.updateRootZ(veg.cmnt, y[I_SM], 100.0);
...
// Get environmental conditions for month "dm"
    ...
    else
    {
        ...
        if (1 == ag.state && ag.getGROWDD() > ag.getGDDSEED(ag.cmnt) && atms.getTAIRN() <= ag.getTKILL(ag.cmnt))
        {
            ...
            refresh();
#endif
            ag.frostDamage(y[I_LEAFC] + y[I_SAPWOODC] + y[I_HEARTWOODC] + y[I_ROOTC] + y[I_SEEDC] + y[I_LABILEC] -
                               ag.getCROPSEEDC(ag.cmnt),
                           y[I_LEAFN] + y[I_SAPWOODN] + y[I_HEARTWOODN] + y[I_ROOTN] + y[I_SEEDN] + y[I_LABILEN] -
    ...
    // if( ag.irrg1950flag == 1 && 3 == ag.state & ag.getGROWDD() >= ag.getGDDSEED(ag.cmnt) && ag.getGROWDD()
    // <=ag.getGDDHARVST(ag.cmnt)) {
    if (ag.irrg1950flag == 1 && ag.state >= 0 & ag.getGROWDD() >= ag.getGDDSEED(ag.cmnt) &&
...
// soil.setNINPUT( ( ag.fertn) );
    ...
    {
        ag.grazing(y[I_LEAFC], y[I_SAPWOODC], y[I_HEARTWOODC], y[I_LABILEC], y[I_LEAFN], y[I_SAPWOODN], y[I_HEARTWOODN],
                   y[I_LABILEN], y[I_ROOTC]);
        ...
        (ag.getGROWDD() >= ag.getGDDSEED(ag.cmnt) && ag.getGROWDD() <= ag.getGDDHARVST(ag.cmnt))) // turflawn
    {
        ...
        {
            frcab = (y[I_LEAFC] + y[I_SAPWOODC] + y[I_HEARTWOODC]) /
                    (y[I_LEAFC] + y[I_SAPWOODC] + y[I_HEARTWOODC] + y[I_ROOTC]);
        }
        cabove = 0.16 * (y[I_LABILEC] * frcab + y[I_LEAFC] + y[I_SAPWOODC] + y[I_HEARTWOODC]);
...
// NOTINSHREE
    ...
    veg.setRLTRC(veg.getRLTRC() * avgfac +
                 (y[I_LTRLC] + y[I_LTRSC] + y[I_LTRHC] + y[I_LTRRC] + y[I_LTRSEEDC]) * (1.0 - avgfac));
    ...
    mdemandn = y[I_ALLOCLN] + y[I_ALLOCSN] + y[I_ALLOCRN] + y[I_ALLOCSEEDN] - y[I_NRESORBL] - y[I_NRESORBS] -
               y[I_NRESORBR] - y[I_NRESORBSEED];
...
int Ttem45::testEquilibrium(const int &pdyr, const int &nyears, const double &vegceq, const double &soilceq,
                            const double &vegneq, const double &soilneq)
{
    //  cout << "diag = " << pdyr << " " << vegceq[pdyr] << " " << soilceq[pdyr] << " " << vegneq[pdyr] << " " <<
    ...
    if (pdyr >= nyears + strteq)
    {
        ...
        if (0 == nfeed && 1 == rheqflag &&
            ...
            && (ctol >= fabs((veg.getVEGC() + y[I_SOLC]) - (vegceq + soilceq))) &&
        ...
        if (ag.state == 0)
        {
            if (1 == nfeed && 1 == rheqflag &&
                ...
                && (ntol >= fabs(soil.yrnin - soil.yrnlost)) &&
                (ntol >= fabs((veg.getVEGN() + y[I_SOLN]) - (vegneq + soilneq))) &&
                (ntol >= fabs((veg.getVEGN() - vegneq))) && (ntol >= fabs((y[I_SOLN] - soilneq)))
        ...
        else if (ag.state == 1)
        {
            if (1 == nfeed && 1 == rheqflag &&
                ...
                && (ntol >= (veg.getVEGN() + y[I_SOLN]) - (vegneq + soilneq)) && (ntol >= (veg.getVEGN() - vegneq)) &&
                (ntol >= (y[I_SOLN] - soilneq))

                && (ctol >= (veg.getVEGC() + y[I_SOLC]) - (vegceq + soilceq)) && (ctol >= (veg.getVEGC() - vegceq)) &&
                (ctol >= (y[I_SOLC] - soilceq)) && (fabs(yrnep - ag.yrformPROD1C) <= 2.0))
...
void Ttem45::updateYearSummary(const int &pdm)
{
    ...
    veg.yrrmaint =
        veg.yrrmaint * (1.0 - favg) + 12.0 * (y[I_RMLEAF] + y[I_RMSAPWOOD] + y[I_RMROOT] + y[I_RMSEED]) * favg;
    ...
    veg.yrltrc =
        veg.yrltrc * (1.0 - favg) + 12.0 * (y[I_LTRLC] + y[I_LTRSC] + y[I_LTRHC] + y[I_LTRRC] + y[I_LTRSEEDC]) * favg;
    ...
    veg.yrnrsorb =
        veg.yrnrsorb * (1.0 - favg) + 12.0 * (y[I_NRESORBL] + y[I_NRESORBS] + y[I_NRESORBR] + y[I_NRESORBSEED]) * favg;

    veg.yrltrn =
        veg.yrltrn * (1.0 - favg) + 12.0 * (y[I_LTRLN] + y[I_LTRSN] + y[I_LTRHN] + y[I_LTRRN] + y[I_LTRSEEDN]) * favg;
