        ...
        if filter_params is not None:
            ...
            if filter_params.get("end_year") is not None:
                try:
                    ...
                    print(
                        RED + "Invalid value for end_year. Must be an integer." + RESET
                    )
...
def main():
    ...
    try:
        ...
        print(
            BOLD
            + GREEN
            + "Running xtran to calculate summary statistics for the input file(s)"
            + RESET
        )
    ...
    if not error_occurred:
        for i, file_path in enumerate(file_list):
            try:
                ...
                print(
                    BLUE
                    + f"File {i+1} of {len(file_list)} processed successfully."
                    + RESET
                )
                print()
            except FileNotFoundError:
