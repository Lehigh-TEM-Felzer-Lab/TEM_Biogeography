...
void Tmcrb45::getvegecd(const string &ecd)
{
    ...
    for (dcmnt = 1; dcmnt < MAXCMNT; ++dcmnt)
    {
        ...
        rhalpha[comtype] = getXMLcmntArrayDouble(infile, "microbeECD", "rhalpha", comtype);

        rhbeta[comtype] = getXMLcmntArrayDouble(infile, "microbeECD", "rhbeta", comtype);

        rhgamma[comtype] = getXMLcmntArrayDouble(infile, "microbeECD", "rhgamma", comtype);

        rhqref[comtype] = getXMLcmntArrayDouble(infile, "microbeECD", "rhqref", comtype);

        rhtref[comtype] = getXMLcmntArrayDouble(infile, "microbeECD", "rhtref", comtype);

        kn2[comtype] = getXMLcmntArrayDouble(infile, "microbeECD", "kn2", comtype);

        moistmin[comtype] = getXMLcmntArrayDouble(infile, "microbeECD", "moistmin", comtype);

        moistopt[comtype] = getXMLcmntArrayDouble(infile, "microbeECD", "moistopt", comtype);

        moistmax[comtype] = getXMLcmntArrayDouble(infile, "microbeECD", "moistmax", comtype);

        docfr[comtype] = getXMLcmntArrayDouble(infile, "microbeECD", "docfr", comtype);

        docdon[comtype] = getXMLcmntArrayDouble(infile, "microbeECD", "docdon", comtype);
...
double Tmcrb45::nminxclm(const int &pdcmnt, const double &soilh2o, const double &soilorgc, const double &soilorgn,
                         const double &availn, const double &ksoil, const int &nopen, const double &immbadd,
                         const double &volac, const double &volan)
{
    //  double gmin;
    double immb;
    double nmin;
...
void Tmcrb45::resetEcds(const int &pcmnt, const double &psiplusc)
{
    kdc = kdb[pcmnt];
...
double Tmcrb45::rhxclm(const double &soilorgc, const double &pdq10, const double &moist, const double &rltrc,
                       const double &volac)
{
...
void Tmcrb45::setDQ10LaRS(const int &pdcmnt, const double &ptair)
{
...
double Tmcrb45::setDQ10LT(const int &pdcmnt, const double &ptair, const double &ptopt)
{
...
double Tmcrb45::setRHMOIST(const int &pdcmnt, const double &pctfldcap, const double &pctwiltpt, const double &pctpor,
                           const double &vsm, const int &moistlim)
{
    ...
    else
    {
        ...
        //      rhxh2o = 0.2+(0.7/(1+exp(-100*(wfps-0.45)))+0.1)*wfps*(1.0-wfps)/(wfps*(1.0-wfps) +
        //      (0.6-wfps)*(0.6-wfps));
        ...
        rhxh2o =
            1.0 + 0.5 * exp(20.0 * ((pctwiltpt + pctfldcap) / 200.0 - vsm)) + 0.5 * exp(20.0 * (vsm - 0.008 * pctpor));
        rhxh2o = 1.0 / rhxh2o;
        //      cout << " vsmopt = " << vsmopt << " " << rhxh2o << " " << vsm << " " << pctwiltpt << " " << pctpor <<
        //      endl; exit(-1);
#endif
    }
...
void Tmcrb45::updateDynamics(const int &pdcmnt, const double &pctfldcap, const double &pctwiltpt, const double &pctpor,
                             const double &soilorgc, const double &soilorgn, const double &soilh2o, const double &vsm,
                             const double &availn, const int &moistlim, const int &tillflag, const double &tillfactor,
                             const double &ksoil, const double &rltrc, const int &nopen, const double &immbadd,
                             const double &volac, const double &volan)
{
    double rhmoist;
    ...
    netnmin = nminxclm(pdcmnt, soilh2o, soilorgc, soilorgn, availn, ksoil, nopen, immbadd, volac, volan);
};
...
double Tmcrb45::yrkd(const int &nfeed, const double &yrltrc, const double &yrltrn, const int &pdcmnt)
{
