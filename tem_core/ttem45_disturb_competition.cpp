...
double Ttem45::rco = (rand() % 2 == 1) ? (double)(rand() % 10 + rand() % 10 + rand() % 10) * -1 / 10
                                       : (double)(rand() % 10 + rand() % 10 + rand() % 10) / 10;
double Ttem45::mco = (rand() % 2 == 1) ? (double)(rand() % 10 + rand() % 10 + rand() % 10) * -1 / 10
                                       : (double)(rand() % 10 + rand() % 10 + rand() % 10) / 10;
double Ttem45::lco = (rand() % 2 == 1) ? (double)(rand() % 10 + rand() % 10 + rand() % 10) * -1 / 10
                                       : (double)(rand() % 10 + rand() % 10 + rand() % 10) / 10;
// PCP code
...
int Ttem45::adapt(const int &numeq, double pstate[], const double &ptol, const int &pdm, const int &pdyr,
        ...
        if (1 == syint)
        {
            while (test != ACCEPT)
            {
                ...
                if (nintmon == maxitmon)
                {
                    ...
                    {
                        pstate[i] = oldstate[i];
                        // if(i == 35) {cout << "pstate = " << pstate[35] << " " << oldstate[35] << " " << nintmon <<
                        // endl;}
                    }
                }
            }
...
int Ttem45::boundcon(double ptstate[], double err[], const double &ptol)
{
...
void Ttem45::cropDynamics(const int &pdm, const int &pdyr, const double &nmax_grow, double pstate[])
{
    ...
    //
    ag.fertn = ZERO;
    //  if((ag.fert1950flag == 1) && (1 == ag.state) && (ag.getGROWDD() >= ag.getGDDSEED(ag.cmnt))&& (ag.getGROWDD() <=
    //  ag.getGDDHARVST(ag.cmnt))|| (1 == ag.getISPERENNIAL(ag.cmnt))) {
    ...
    // if( (ag.fert1950flag == 1) && (3 == ag.state) && (ag.getGROWDD() >= ag.getGDDSEED(ag.cmnt)) && (ag.getGROWDD() <=
    // ag.getGDDHARVST(ag.cmnt))){
    //    cout << "entering second conditional " << endl;
    //    ag.fertn = 10.0/6.0;
    //    }

    atms.setLWOUTD(atms.lwrad(atms.getTAIRD(), atms.getVPR(), atms.getNIRR(), atms.getGIRR()));

    atms.setLWOUTN(atms.lwrad(atms.getTAIRN(), atms.getVPR(), atms.getNIRR(), atms.getGIRR()));
    // nirrn already set
    ...
    veg.aerodynamics(ag.cmnt, atms.getWS10());
    ...
    {
        veg.pen.hydraulics(ag.cmnt, veg.getPHEN(ag.cmnt), veg.getLAI(), veg.getSLA(ag.cmnt), veg.getSAPWOODC(),
                           veg.getROOTC(), soil.getAVLH2O(), soil.getAWCAPMM(), atms.getPREC());
    }

    veg.rxTLaRS(ag.cmnt, atms.getTAIR(), veg.getRATREF(ag.cmnt));
    if (0 == o3flag)
    {
        veg.setFOZONE(1.0);
    }
    ...
    if ((ag.getGROWDD() >= ag.getGDDSEED(ag.cmnt)) || (1 == ag.getISPERENNIAL(ag.cmnt)))
    {
        veg.litterresp(ag.cmnt, atms.getNDAYS(pdm));
    }
    ...
    microbe.updateDynamics(ag.cmnt, soil.getPCTFLDCAP(), soil.getPCTWILTPT(), soil.getPCTPOR(), pstate[I_SOLC],
                           pstate[I_SOLN], pstate[I_SM], pstate[I_VSM], pstate[I_AVLN], moistlim, ag.tillflag,
                           ag.getTILLFACTOR(ag.cmnt), soil.getKH2O(), veg.getRLTRC(), nopen, ag.getIMMBADD(),
                           ag.getVOLAC(), ag.getVOLAN());
    ...
    if ((ag.getGROWDD() >= ag.getGDDSEED(ag.cmnt)) || (1 == ag.getISPERENNIAL(ag.cmnt)))
    {
        veg.nupxclm(ag.cmnt, nmax_grow, pstate[I_SM], pstate[I_AVLN], veg.getRMT(), soil.getKH2O(), veg.getFOZONE(),
                    pstate[I_ROOTC]);

        veg.gppxclm(ag.cmnt, atms.getNDAYS(pdm), atms.getCO2(), atms.getPAR(), atms.getVPR(), atms.getVPDD(),
                    atms.getDAYL(), veg.pen.getKEXT(ag.cmnt), microbe.getRH(), atms.getPREC(), veg.getVEGC());

        //----------------------------------------------//
        //  determine allocation fluxes
        veg.allocate(ag.cmnt, atms.getNDAYS(pdm), nfeed, pdm, ag.getGROWDD(), ag.state);
        ...
        //      if(initFlag == 1) { cout << "cropveg = " << soil.getSONINP() << " " << veg.getVEGNINP() << " " <<
        //      soil.getEET() << endl;}
#endif

        veg.updateDynamics(ag.cmnt, soil.getNINPUT(), pstate[I_AVLN], nfeed, ag.state, ag.getISPERENNIAL(ag.cmnt),
                           ag.fert1950flag, microbe.getNETNMIN(), ag.fertn, soil.getSONINP());
    ...
    veg.petsw(ag.cmnt, pdm, atms.getNDAYS(pdm), atms.getDAYL(), atms.getTAIRD(), atms.getTAIRN(), atms.getCO2(),
              atms.getNIRRN(), atms.getLWOUTD(), atms.getLWOUTN(), atms.getVPR(), atms.getVPDD(), atms.getVPDN(),
              soil.getSNOWPACK(), atms.getPREC(), veg.getESOILMMMO(), soil.getAVLH2O(), elev);

    veg.deltafo3(ag.cmnt, atms.getAOT40());

    soil.updateHydrology(elev, atms.getTAIR(), atms.getPREVTAIR(), atms.getPREV2TAIR(), atms.getRAIN(), veg.getPET(),
                         soil.getAVLH2O(), pstate[I_RGRW], pstate[I_SGRW], ag.irrgflag, ag.irrigate, pdm);

    // if(initFlag == 1) {cout << "eet in cropveg = " << soil.getEET() << endl;}

    soil.updateDOCLEACH(pstate[I_DOC], pstate[I_SM]);
    ...
    if (1 == avlnflag)
    {
        ...
        soil.updateNLosses(ag.cmnt,
                           ...
                           (soil.getRPERC() + soil.getSPERC()), pstate[I_AVLN], pstate[I_SM]);
        ...
        //if(nseed != 0.0 || nprod != 0.0){ cout << "cropseedn = " << pdm << " " << nseed << " " << nprod  << " " <<
        ag.getCROPRESIDUEFLXN() << endl;}

        //if(pdm == 9) {cout << "diag = " << pdm << " " << soil.getNINPUT() << " " << soil.getSONINP() << " " <<
        veg.getVEGNINP() << " " << nseed << " " << soil.getNLOST() << " " << soil.getLCHDON() << " " <<
        ag.getCROPRESIDUEFLXN() << " " << nprod << " " << nin << " " << nout << endl;}

           if(nout < nin)
           {
        //    if( ag.getCROPPRODN() > 0.0) {cout << "too much NIN " << nin - nout << " " << ag.getCROPPRODN() << endl;}
        //    if(nseed != 0.0 || nprod != 0.0) cout << "too much NIN " << nin - nout << " " << nin << " " << nout <<
        endl;
        ...
        //      soil.SetNINPUT(soil.getNINPUT() - (nin - nout));
           ...
           if(nout > nin)
           {
        //    if(nseed != 0.0 || nprod != 0.0) cout << "too much NOUT " << nout - nin << " " << nin << " " << nout << "
        " << soil.getNLOST() << " " << soil.getLCHDON() << " " << ag.getCROPRESIDUEFLXN() << " " << nprod << endl;
        ...
        //        soil.setNLOST(soil.getNLOST() - (nout - nin));
           ...
           { */
        // cout << "setNLOST in crops = " << soil.getNLOST() << " " << soil.getDENITR(ag.cmnt) << " " <<
        // microbe.getGMIN() << " " << veg.getDENITR() << endl; cout << "gmin in crops = " << microbe.getGMIN() << " "
        // << soil.getNLOST() << " " << soil.getDENITR(ag.cmnt) << " " << veg.getDENITR() << endl;
...
void Ttem45::delta(const int &pdm, const int &pdyr, const double &nmax_grow, double pstate[], double pdstate[])
{
#ifdef DEBUG_CTEM
    move(DEBUG_ROW, 1);
    ...
    //  cout << "leafc = " << pdstate[I_LEAFC] << " " << veg.getALLOCLC() << " " << veg.getRMLEAF() << " " <<
    //  veg.getLTRLC() << " " << veg.getLEAFC() << " " << pstate[0] << endl;
    ...
    pdstate[I_LABILEC] =
        veg.getGPP() - veg.getALLOCLC() - veg.getALLOCSC() - veg.getALLOCRC() - veg.getALLOCSEEDC() - veg.getRGRWTH();
    ...
    pdstate[I_SOLC] = veg.getLTRLC()
                      ...
                      + veg.getLTRSC() + veg.getLTRHC() + veg.getLTRRC() + veg.getLTRSEEDC() +
                      ag.getSLASHC()
                      //                    + 0.865 * ag.getSLASHC()
                      - ag.getSCONVRTFLXC() - microbe.getDOCPROD() - microbe.getRH();
    //  ag.setSLASHC(0.0);

    //  cout << "soil = " << pstate[I_SOLC] << " " << pdstate[I_SOLC] << " " << veg.getLTRLC() << " " << veg.getLTRSC()
    //  << " " << veg.getLTRHC() << " " << veg.getLTRRC() << " " << veg.getLTRSEEDC() << " " << ag.getSLASHC() << " " <<
    //  ag.getSCONVRTFLXC() << " " << microbe.getRH() << " " << microbe.getDOCPROD() << endl;
    ...
    //  cout << "pdstate = " << pdstate[I_ROOTN] << " " << veg.getALLOCRN() << " " << veg.getNRESORBR() << " " <<
    //  veg.getLTRRN() << " " << y[I_ROOTN] << endl;
    ...
    pdstate[I_SOLN] = veg.getLTRLN()
                      ...
                      //                    + 0.865 * ag.getSLASHN()
                      + veg.getLTRSN() + veg.getLTRHN() + veg.getLTRRN() + veg.getLTRSEEDN() + ag.getSLASHN() +
                      soil.getSONINP() - microbe.getDONPROD() - ag.getSCONVRTFLXN() - ag.getNSRETENT() -
                      microbe.getNETNMIN();

    // if(initFlag == 1)   cout << "deltasoln = " << pdstate[I_SOLN] << " " << ag.getNSRETENT() << " " << veg.getLTRLN()
    // << " " << veg.getLTRSN() << " " << veg.getLTRHN() << " " << veg.getLTRRN() << " " << veg.getLTRSEEDN() << " " <<
    // ag.getSLASHN() << " "  << soil.getSONINP() << " " << microbe.getDONPROD() << " " << ag.getSCONVRTFLXN() << " " <<
    // microbe.getNETNMIN() << endl;
    //    if(initFlag == 1) {cout << "soninp = " << soil.getSONINP() << " " << y[I_SOLN] << endl;}

    pdstate[I_LABILEN] = veg.getNUPTAKE() + veg.getVEGNINP() - veg.getALLOCLN() - veg.getALLOCSN() - veg.getALLOCRN() -
                         veg.getALLOCSEEDN() + veg.getNRESORBL() + veg.getNRESORBS() + veg.getNRESORBR() +
                         veg.getNRESORBSEED();

    // cout << "labilen = " << pdstate[I_LABILEN] << " " << veg.getNUPTAKE() << " " << veg.getVEGNINP() << " " <<
    // veg.getNRESORBL() << " " << veg.getNRESORBS() << " " << veg.getNRESORBR() << " " << veg.getNRESORBSEED() << " "
    // <<  veg.getALLOCLN() << " " << veg.getALLOCSN() << " " << veg.getALLOCRN() << " " << veg.getALLOCSEEDN() << endl;

#else
    pdstate[I_SOLN] = veg.getLTRLN() + veg.getLTRSN() + veg.getLTRHN() + veg.getLTRRN() + veg.getLTRSEEDN() +
                      ag.getSLASHN() - ag.getSCONVRTFLXN() - ag.getNSRETENT() - microbe.getNETNMIN();

    pdstate[I_LABILEN] = veg.getNUPTAKE() - veg.getALLOCLN() - veg.getALLOCSN() - veg.getALLOCRN() -
                         veg.getALLOCSEEDN() + veg.getNRESORBL() + veg.getNRESORBS() + veg.getNRESORBR() +
                         veg.getNRESORBSEED();

#endif

    pdstate[I_AVLN] = soil.getNINPUT() + microbe.getNETNMIN() +
                      ag.getVOLAN()
                      //                    + veg.getVEGNINP()
                      - veg.getNUPTAKE() - soil.getNLOST();

    // cout << "pdavln = " << pdstate[I_AVLN] << " " << soil.getNINPUT() << " " << microbe.getNETNMIN() << " " <<
    // ag.getVOLAN() << " " << veg.getNUPTAKE() << " " << soil.getNLOST() << " " << pstate[I_AVLN] << endl;
    ...
    //  {
    pdstate[I_SM] =
        soil.getSNOWINF() + atms.getRAIN() + ag.irrigate - soil.getEET() - soil.getRPERC() - soil.getSPERC();
    //  }
    // if (ag.irrigate > 0.0) {cout << "ag.irrigate  = " << ag.irrigate << endl;}
    //  cout << "delta sm = " << pdstate[I_SM] << " " << pstate[I_SM] << " " << soil.getEET() << " " << atms.getRAIN()
    //  << endl;
...
void Ttem45::ECDsetODEstate(const int &pdcmnt, const double &psiplusc)
{
...
void Ttem45::getenviron(void)
{
    ...
    soil.setSNOWINF(soil.snowmelt(elev, atms.getTAIR(), atms.getPREVTAIR(), soil.getPREVSPACK()));
...
void Ttem45::getsitecd(const int &dv, const string &ecd)
{
    ...
    veg.getXMLsiteRootNode(fecd[dv], "siteECD", version, sitename, sitecol, siterow, developer, updated);

    veg.cmnt = veg.getXMLsiteCommunityNode(fecd[dv], "siteECD", description);

    leafcb[veg.cmnt] = veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "leafcb", veg.cmnt);

    sapwoodcb[veg.cmnt] = veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "sapwoodcb", veg.cmnt);
    //  cout << "getsitecd = " << veg.cmnt << " " << sapwoodcb[veg.cmnt] << endl;

    heartwoodcb[veg.cmnt] = veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "heartwoodcb", veg.cmnt);

    rootcb[veg.cmnt] = veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "rootcb", veg.cmnt);

    seedcb[veg.cmnt] = veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "seedcb", veg.cmnt);

    labilecb[veg.cmnt] = veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "labilecb", veg.cmnt);

    leafnb[veg.cmnt] = veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "leafnb", veg.cmnt);

    sapwoodnb[veg.cmnt] = veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "sapwoodnb", veg.cmnt);

    heartwoodnb[veg.cmnt] = veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "heartwoodnb", veg.cmnt);

    rootnb[veg.cmnt] = veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "rootnb", veg.cmnt);

    seednb[veg.cmnt] = veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "seednb", veg.cmnt);

    labilenb[veg.cmnt] = veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "labilenb", veg.cmnt);

    solcb[veg.cmnt] = veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "solcb", veg.cmnt);

    solnb[veg.cmnt] = veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "solnb", veg.cmnt);

    avlnb[veg.cmnt] = veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "avlnb", veg.cmnt);

    veg.setCMAX1B(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "vegcmax1b", veg.cmnt), veg.cmnt);

    veg.setTAULEAF(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "vegtauleaf", veg.cmnt), veg.cmnt);

    veg.setTAUSAPWOOD(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "vegtausapwood", veg.cmnt), veg.cmnt);

    veg.setTAUHEARTWOOD(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "vegtauheartwood", veg.cmnt), veg.cmnt);

    veg.setTAUROOT(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "vegtauroot", veg.cmnt), veg.cmnt);

    veg.setTAUSEED(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "vegtauseed", veg.cmnt), veg.cmnt);

    veg.setKRA(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "vegkra", veg.cmnt), veg.cmnt);

    microbe.setKDB(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "microbekdb", veg.cmnt), veg.cmnt);

    microbe.setLCCLNC(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "microbelcclnc", veg.cmnt), veg.cmnt);

    microbe.setPROPFTOS(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "microbepropftos", veg.cmnt), veg.cmnt);

    veg.setNMAX1B(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "vegnmax1b", veg.cmnt), veg.cmnt);

    microbe.setNUPB(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "microbenupb", veg.cmnt), veg.cmnt);

    soil.setNLOSS(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "soilnloss", veg.cmnt), veg.cmnt);

    soil.setDENITR(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "soildenitr", veg.cmnt), veg.cmnt);

    microbe.setCNSOIL(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "microbecnsoil", veg.cmnt), veg.cmnt);

    veg.setCNLTR(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "vegcnltr", veg.cmnt), veg.cmnt);

    veg.setO3PARA(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "o3para", veg.cmnt), veg.cmnt);

    veg.setO3PARB(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "o3parb", veg.cmnt), veg.cmnt);

    veg.setO3PARC(veg.getXMLcmntArrayDouble(fecd[dv], "siteECD", "o3parc", veg.cmnt), veg.cmnt);
...
void Ttem45::massbal(const int &ichrt)
{
    ...
    if ((y[I_SM][ichrt] - prevy[I_SM][ichrt]) != (soil.getSNOWINF() + atms.getRAIN() + y[I_AGIRRIG][ichrt] -
                                                  y[I_RPERC][ichrt] - y[I_EET][ichrt] - y[I_SPERC][ichrt]))
    {
        y[I_SPERC][ichrt] = soil.getSNOWINF() + atms.getRAIN() + y[I_AGIRRIG][ichrt] - y[I_RPERC][ichrt] -
                            y[I_EET][ichrt] - y[I_SM][ichrt] + prevy[I_SM][ichrt];
    }
    ...
    {
        y[I_LTRSC][ichrt] = y[I_ALLOCSC][ichrt] - y[I_ALLOCHC][ichrt] - y[I_RMSAPWOOD][ichrt] - y[I_SAPWOODC][ichrt] +
                            prevy[I_SAPWOODC][ichrt];
    }
    ...
    if (y[I_LABILEC][ichrt] - prevy[I_LABILEC][ichrt] != y[I_GPP][ichrt] - y[I_ALLOCLC][ichrt] - y[I_ALLOCSC][ichrt] -
                                                             y[I_ALLOCRC][ichrt] - -y[I_ALLOCSEEDC][ichrt] -
                                                             y[I_RVGRW][ichrt])
    {
    ...
    if (y[I_SOLC][ichrt] - prevy[I_SOLC][ichrt] != y[I_LTRLC][ichrt] + y[I_LTRSC][ichrt] + y[I_LTRHC][ichrt] +
                                                       y[I_LTRRC][ichrt] + y[I_LTRSEEDC][ichrt] + ag.getSLASHC() -
                                                       ag.getSCONVRTFLXC() - y[I_RH][ichrt] - y[I_DOCPROD][ichrt])
    {
        y[I_RH][ichrt] = y[I_LTRLC][ichrt] + y[I_LTRSC][ichrt] + y[I_LTRHC][ichrt] + y[I_LTRRC][ichrt] +
                         y[I_LTRSEEDC][ichrt] + ag.getSLASHC() - ag.getSCONVRTFLXC() - y[I_DOCPROD][ichrt] -
                         y[I_SOLC][ichrt] + prevy[I_SOLC][ichrt];
    }
    ...
    if (y[I_LABILEN][ichrt] - prevy[I_LABILEN][ichrt] !=
        y[I_VNUP][ichrt] + y[I_NRESORBL][ichrt] + y[I_NRESORBS][ichrt] + y[I_NRESORBR][ichrt] +
            y[I_NRESORBSEED][ichrt] + y[I_NFIXS][ichrt] - y[I_ALLOCLN][ichrt] - y[I_ALLOCSN][ichrt] -
            y[I_ALLOCRN][ichrt] - y[I_ALLOCSEEDN][ichrt])
    {
        y[I_NRESORBL][ichrt] = y[I_ALLOCLN][ichrt] + y[I_ALLOCSN][ichrt] + y[I_ALLOCRN][ichrt] +
                               y[I_ALLOCSEEDN][ichrt] + y[I_NFIXS][ichrt] - y[I_VNUP][ichrt] + y[I_LABILEN][ichrt] -
                               prevy[I_LABILEN][ichrt] - y[I_NRESORBS][ichrt] - y[I_NRESORBR][ichrt] -
    ...
    {
        y[I_LTRSN][ichrt] = y[I_ALLOCSN][ichrt] - y[I_ALLOCHN][ichrt] - y[I_NRESORBS][ichrt] - y[I_SAPWOODN][ichrt] +
                            prevy[I_SAPWOODN][ichrt];
    }
    ...
    {
        y[I_LTRSEEDN][ichrt] =
            y[I_ALLOCSEEDN][ichrt] - y[I_NRESORBSEED][ichrt] - y[I_SEEDN][ichrt] + prevy[I_SEEDN][ichrt];
    }
...
#ifdef OPENN
    if (y[I_SOLN][ichrt] - prevy[I_SOLN][ichrt] != y[I_LTRLN][ichrt] + y[I_LTRSN][ichrt] + y[I_LTRHN][ichrt] +
                                                       y[I_LTRRN][ichrt] + y[I_LTRSEEDN][ichrt] + ag.getSLASHN() +
                                                       y[I_NFIXN][ichrt] - y[I_DONPROD][ichrt] - ag.getSCONVRTFLXN() -
                                                       ag.getNSRETENT() - y[I_NMIN][ichrt])
    {
        y[I_NMIN][ichrt] = y[I_LTRLN][ichrt] + y[I_LTRSN][ichrt] + y[I_LTRHN][ichrt] + y[I_LTRRN][ichrt] +
                           y[I_LTRSEEDN][ichrt] + ag.getSLASHN() + y[I_NFIXN][ichrt] - y[I_DONPROD][ichrt] -
                           ag.getSCONVRTFLXN() - ag.getNSRETENT() - y[I_SOLN][ichrt] + prevy[I_SOLN][ichrt];
    }
  ...
  {
      y[I_NMIN][ichrt] = y[I_LTRLN][ichrt] + y[I_LTRSN][ichrt] + y[I_LTRHN][ichrt] + y[I_LTRRN][ichrt] +
                         y[I_LTRSEEDN][ichrt] + ag.getSLASHN() - ag.getSCONVRTFLXN() - ag.getNSRETENT() -
                         y[I_SOLN][ichrt] + prevy[I_SOLN][ichrt];
  }
  ...
  if (y[I_SOLN][ichrt] < ZERO)
  {
    ...
    {
        //    y[I_NLST] = ZERO;
        y[I_NINP][ichrt] = y[I_NLST][ichrt] - y[I_NMIN][ichrt] - ag.getVOLAN() + y[I_VNUP][ichrt] + y[I_AVLN][ichrt] -
                           prevy[I_AVLN][ichrt];
    }
    ...
    {
        //    y[I_NINP] = ZERO;
        y[I_NLST][ichrt] = y[I_NINP][ichrt] + y[I_NMIN][ichrt] + ag.getVOLAN() - y[I_VNUP][ichrt] - y[I_AVLN][ichrt] +
                           prevy[I_AVLN][ichrt];
    }
};
...
int Ttem45::monthlyTransient(const int &pdyr, const int &pdm, const double &ptol, const int &ichrt, ofstream &rflog1)
{
    endeq = 0;
    initFlag = 1;
    ...
    {
        y[I_VSM][ichrt] = soil.updateRootZ(veg.cmnt, y[I_SM][ichrt], y[I_ROOTC][ichrt]);

        veg.resetEcds(veg.cmnt, soil.getPSIPLUSC());
    }
    else
    {
        y[I_VSM][ichrt] = soil.updateRootZ(ag.cmnt, y[I_SM][ichrt], y[I_ROOTC][ichrt]);
...
void Ttem45::natvegDynamics(const int &pdm, const double &nmax_grow, double pstate[])
{
    ...
    soil.setNLOST(0.0);
    soil.setNINPUT(ag.getNRETENT());
    //  cout << "avalh2o = " << soil.getAVLH2O() << " " << soil.getVSM() << " " << soil.getSWP() << " " <<
    //  soil.getMOIST() << " " << soil.getKH2O() << endl; cout << "water terms = " << soil.getSNOWINF() << " " <<
    //  atms.getRAIN() <<  " " << ag.irrigate << " " << soil.getEET() << " " << soil.getRPERC() << " " <<
    //  soil.getSPERC() << endl;
    ...
    veg.aerodynamics(veg.cmnt, atms.getWS10());
    ...
    veg.pen.setR_SS(exp(8.206 - 4.205 * sqrt(surf_wetness * prob))); // using formulation from SiB2
                                                                     //    }
    //    veg.pen.setR_SS((11.489*exp(-11.67*soil.getVSM()))* exp(8.206 - 4.205*sqrt(surf_wetness*prob)) ); // using
    //    formulation from SiB2 veg.pen.setR_SS((78.935*exp(-20.7*soil.getVSM()))* exp(8.206
    //    - 4.205*sqrt(surf_wetness*prob)) ); // using formulation from SiB2
    //    veg.pen.setR_SS((-4.64*log(soil.getVSM())-6.5404)* exp(8.206 - 4.205*sqrt(surf_wetness*prob)) ); // using
    //    formulation from SiB2
#endif

    veg.pen.hydraulics(veg.cmnt, veg.getPHEN(veg.cmnt), veg.getLAI(), veg.getSLA(veg.cmnt), veg.getSAPWOODC(),
                       veg.getROOTC(), soil.getAVLH2O(), soil.getAWCAPMM(), atms.getPREC());

    veg.rxTLaRS(veg.cmnt, atms.getTAIR(), veg.getRATREF(veg.cmnt));
    if (0 == o3flag)
    {
        veg.setFOZONE(1.0);
    }
    ...
    //----------------------------------------------//
    //  determine litterfall and respiration fluxes
    veg.litterresp(veg.cmnt, atms.getNDAYS(pdm));
    ...
    microbe.updateDynamics(veg.cmnt, soil.getPCTFLDCAP(), soil.getPCTWILTPT(), soil.getPCTPOR(), pstate[I_SOLC],
                           pstate[I_SOLN], pstate[I_SM], pstate[I_VSM], pstate[I_AVLN], moistlim, tillflag,
                           ag.getTILLFACTOR(veg.cmnt), soil.getKH2O(), veg.getRLTRC(), nopen, ag.getIMMBADD(),
                           ag.getVOLAC(), ag.getVOLAN());
    ...
    move(DEBUG_ROW + 1, 1);
    printw(" aerodynamic resistances (raa,rac) = %8.21f  %8.21f, lai = %8.21f ", veg.pen.getR_AA(), veg.pen.getR_AC(),
           veg.getLAI());
    refresh();
#endif
    ...
    veg.nupxclm(veg.cmnt, nmax_grow, pstate[I_SM], pstate[I_AVLN], veg.getRMT(), soil.getKH2O(), veg.getFOZONE(),
                pstate[I_ROOTC]);

    veg.gppxclm(veg.cmnt, atms.getNDAYS(pdm), atms.getCO2(), atms.getPAR(), atms.getVPR(), atms.getVPDD(),
                atms.getDAYL(), veg.pen.getKEXT(veg.cmnt), microbe.getRH(), atms.getPREC(), veg.getVEGC());

    //  if(atms.getPREC() > 10 && veg.pen.getFWS() < .9) {cout << "fws = " << veg.pen.getFWS() << " " << atms.getPREC()
    //  << endl;}
    ...
    veg.allocate(veg.cmnt, atms.getNDAYS(pdm), nfeed, pdm, ag.getGROWDD(), ag.state);

    //  cout << "diag = " << veg.getALLOCLC() << " " << veg.getALLOCSC() << " " << veg.getALLOCRC() << " " <<
    //  veg.getALLOCSEEDC() << " " << veg.getRGRWTH() << endl;
...
//  if(initFlag == 1) { cout << "natveg = " << soil.getSONINP() << " " << veg.getVEGNINP() << " " << soil.getEET() <<
//  endl; }
#endif
    //
    ...
    else
    {
        ...
        veg.updateDynamics(veg.cmnt, soil.getNINPUT(), pstate[I_AVLN], nfeed, agstate, perennial, fertflag,
                           microbe.getNETNMIN(), ag.fertn, soil.getSONINP());
        soil.setSONINP(veg.getSONINPUT());

        // if(initFlag == 1) {cout << "post = " << soil.getSONINP() << endl;}
        // if(initFlag == 1) {cout << "diag = " << soil.getSONINP() << " " << veg.getVEGNINP() << " " << veg.getDENITR()
        // << endl;}
    ...
    veg.setESOILMMMO(0.0);
    veg.petsw(veg.cmnt, pdm, atms.getNDAYS(pdm), atms.getDAYL(), atms.getTAIRD(), atms.getTAIRN(), atms.getCO2(),
              atms.getNIRRN(), atms.getLWOUTD(), atms.getLWOUTN(), atms.getVPR(), atms.getVPDD(), atms.getVPDN(),
              soil.getSNOWPACK(), atms.getPREC(), veg.getESOILMMMO(), soil.getAVLH2O(), elev);

    if ((soil.getAVLH2O() < .5 * soil.getAWCAPMM()) && (veg.getPESOILW() > 0.5 * atms.getPREC()))
    {
        //  cout << "met condition " << soil.getAVLH2O() << " " << soil.getAWCAPMM() << " " << veg.getPESOILW() << " "
        //  << atms.getPREC() << endl;
    ...
    // cout << "petsw = " << veg.cmnt << " " << atms.getNDAYS(pdm) << " " << atms.getDAYL() << " " << atms.getTAIRD() <<
    // " " << atms.getTAIRN() << " " << atms.getCO2() << " " << atms.getNIRRN() << " " << atms.getLWOUTD() << " " <<
    // atms.getLWOUTN() << " " << atms.getVPR() << " " << atms.getVPDD() << " " << atms.getVPDN() << " " <<
    // soil.getSNOWPACK() << " " << atms.getPREC() << " " << veg.getESOILMMMO() << " " << elev << endl;
    ...
    veg.deltafo3(veg.cmnt, atms.getAOT40());

    soil.updateHydrology(elev, atms.getTAIR(), atms.getPREVTAIR(), atms.getPREV2TAIR(), atms.getRAIN(), veg.getPET(),
                         soil.getAVLH2O(), pstate[I_RGRW], pstate[I_SGRW], irrgflag, ag.irrigate, pdm);
    ...
    /* if(veg.cmnt == 5)
    {
      soil.setSONINP((veg.getNNF(veg.cmnt)*0.102 * (12.0*((soil.getEET()/soil.getREET())*90.0)/10.0)+ 0.524
    )/(10.0*12.0)); veg.setVEGNINP(((1.0-veg.getNNF(veg.cmnt))*0.102 *
    (12.0*((soil.getEET()/soil.getREET())*90.0)/10.0)+ 0.524 )/(10.0*12.0));
    }
    if(veg.cmnt == 6)
    {
      soil.setSONINP((veg.getNNF(veg.cmnt)*0.102 * (12.0*((soil.getEET()/(1.0+soil.yreet))*140.0/12.0)/10.0)+ 0.524
    )/(10.0*12.0)); veg.setVEGNINP(((1.0-veg.getNNF(veg.cmnt))*0.102 *
    (12.0*((soil.getEET()/(1.0+soil.yreet))*140.0/12.0)/10.0)+ 0.524 )/(10.0*12.0));
    ...
    //  cout << "evapcora = " << wevap <<  " " << " " << rfrac << " " << soil.getRRUN() << " " << soil.getSRUN() <<
    endl;
      }
    //  }  */
    ...
    if (1 == avlnflag)
    {
        ...
        {
            // cout << "setNLOST in natveg = " << soil.getNLOST() << " " << soil.getDENITR(ag.cmnt) << " " <<
            // microbe.getGMIN() << " " << veg.getDENITR() << endl; cout << "gmin = " << microbe.getGMIN() << " " <<
            // soil.getNLOST() << " " << soil.getDENITR(ag.cmnt) << " " << veg.getDENITR() << endl;
            soil.setNLOST(soil.getNLOST() + soil.getDENITR(veg.cmnt) * (0.01 * microbe.getGMIN() + veg.getDENITR()));
        }
        //   else {
        // cout << "uhoh = " << soil.getNLOST() << " " <<  soil.getLCHDON() << " " << soil.getNINPUT() << " " <<
        // soil.getSONINP() << " " <<  veg.getVEGNINP() << endl;
        //    }
        //   else if (soil.getNLOST() + soil.getLCHDON() > soil.getNINPUT() + soil.getSONINP() + veg.getVEGNINP())
        //   {
        //      soil.setNINPUT(soil.getNINPUT() + (soil.getNLOST() + soil.getLCHDON()) - (soil.getNINPUT() +
        //      soil.getSONINP() + veg.getVEGNINP()));
        //   }
    ...
    // cout << "diag = " << inout << " " << veg.getVEGNINP() << " " << soil.getSONINP() << " " << soil.getNINPUT() << "
    // " << soil.getNLOST() <<  " " << soil.getLCHDON() << endl;
...
void Ttem45::pcdisplayMonth(const int &dyr, const int &pdm)
{
    ...
    printw("%3d-%2d %7.1lf %7.1lf %7.1lf %7.2lf %7.2lf %7.2lf %7.2lf %7.3lf %7.3lf %7.3lf %7.1lf %7.1lf %7.1lf %7.2lf "
           "%7.2lf %7.2lf %7.3lf %7.3lf",
           dyr, (pdm + 1), outevar[0], outevar[1], outevar[2], outevar[3], outevar[4], outevar[5], outevar[6],
           outevar[7], outevar[8], outevar[9], outwvar[0], outwvar[1], outwvar[2], outwvar[3], outwvar[4], outwvar[5],
           outwvar[6], outwvar[7]);

    if (pdm == 11)
    {
        move(15, 1);
        printw(" SUM   %7.0lf %7.0lf %7.1lf %7.1lf %7.1lf %7.1lf %7.1lf %7.2lf %7.2lf %7.2lf %7.0lf %7.0lf %7.0lf "
               "%7.1lf %7.1lf %7.1lf %7.2lf %7.2lf",
               outevarsum[0], outevarsum[1], outevarsum[2], outevarsum[3], outevarsum[4], outevarsum[5], outevarsum[6],
               outevarsum[7], outevarsum[8], outevarsum[9], outwvarsum[0], outwvarsum[1], outwvarsum[2], outwvarsum[3],
               outwvarsum[4], outwvarsum[5], outwvarsum[6], outwvarsum[7]);

        move(16, 1);
        printw(" MAX   %7.1lf %7.1lf %7.1lf %7.2lf %7.2lf %7.2lf %7.2lf %7.3lf %7.3lf %7.3lf %7.1lf %7.1lf %7.1lf "
               "%7.2lf %7.2lf %7.2lf %7.3lf %7.3lf",
               outevarmax[0], outevarmax[1], outevarmax[2], outevarmax[3], outevarmax[4], outevarmax[5], outevarmax[6],
               outevarmax[7], outevarmax[8], outevarmax[9], outwvarmax[0], outwvarmax[1], outwvarmax[2], outwvarmax[3],
               outwvarmax[4], outwvarmax[5], outwvarmax[6], outwvarmax[7]);

        move(17, 1);
        printw(" AVG   %7.1lf %7.1lf %7.1lf %7.2lf %7.2lf %7.2lf %7.2lf %7.3lf %7.3lf %7.3lf %7.1lf %7.1lf %7.1lf "
               "%7.2lf %7.2lf %7.2lf %7.3lf %7.3lf",
               outevaravg[0], outevaravg[1], outevaravg[2], outevaravg[3], outevaravg[4], outevaravg[5], outevaravg[6],
               outevaravg[7], outevaravg[8], outevaravg[9], outwvaravg[0], outwvaravg[1], outwvaravg[2], outwvaravg[3],
               outwvaravg[4], outwvaravg[5], outwvaravg[6], outwvaravg[7]);

        move(18, 1);
        printw(" MIN   %7.1lf %7.1lf %7.1lf %7.2lf %7.2lf %7.2lf %7.2lf %7.3lf %7.3lf %7.3lf %7.1lf %7.1lf %7.1lf "
               "%7.2lf %7.2lf %7.2lf %7.3lf %7.3lf",
               outevarmin[0], outevarmin[1], outevarmin[2], outevarmin[3], outevarmin[4], outevarmin[5], outevarmin[6],
               outevarmin[7], outevarmin[8], outevarmin[9], outwvarmin[0], outwvarmin[1], outwvarmin[2], outwvarmin[3],
               outwvarmin[4], outwvarmin[5], outwvarmin[6], outwvarmin[7]);
    }
    refresh();
};
...
void Ttem45::rkbs(const int &numeq, double pstate[], double &pdt, const int &pdm, const int &pdyr,
                  const double &nmax_grow)
{
    ...
    if (calc_dz1_flag != 0)
    {
        ...
        //       cout << "delta1 = " << pstate[35] << " " << dz1[35] << " " << sapwoodcb[11] << " " << sapwoodcb[4] <<
        //       endl;
    }
    ...
    //  cout << "error = " << error[35] << " " << dum2[35] << " " << dum3[35] << " " << veg.cmnt << " " <<
    //  sapwoodcb[veg.cmnt] << " " << sapwoodcb[11] << " " << sapwoodcb[4] << endl;
};
...
void Ttem45::rkf(const int &numeq, double pstate[], double &pdt, const int &pdm, const int &pdyr,
                 const double &nmax_grow)
{
...
void Ttem45::setELMNTecd(const int &pdcmnt, const double &psiplusc)
{
...
void Ttem45::step(const int &numeq, double pstate[], double pdstate[], double ptstate[], double &pdt)
{
    ...
    //    cout << "diag = " << ptstate[0] << " " << ptstate[1] << " " << ptstate[2] << " " << ptstate[3] << " " <<
    //    ptstate[4] << " " << ptstate[5] << " " << ptstate[6] << " " << ptstate[7] << " " << ptstate[8] << " " <<
    //    ptstate[9] << " " << ptstate[10] << " " << ptstate[11] << " " << ptstate[12] << " " << ptstate[13] << " " <<
    //    ptstate[14] << " " << ptstate[15] << " " << ptstate[16] << " " << ptstate[17] << " " << ptstate[18] << " " <<
    //    ptstate[19] << " " << ptstate[20] << " " << ptstate[21] << " " << ptstate[22] << " " << ptstate[23] << " " <<
    //    ptstate[24] << " " << ptstate[25] << " " << ptstate[26] << " " << ptstate[27] << " " << ptstate[28] << " " <<
    //    ptstate[29] << " " << ptstate[30] << " " << ptstate[31] << " " << ptstate[32] << " " << ptstate[33] << " " <<
    //    ptstate[34] << " " << ptstate[35] << endl;
};
...
int Ttem45::stepmonth(const int &pdyr, const int &pdm, int &intflag, const double &ptol, const int &ichrt,
                      ofstream &rflog1)
{
    ...
    double cwdloss = 0.0;
    // cout << "entering stepmonth" << " " << y[I_SAPWOODC] << " " << veg.cmnt << " " << ag.cmnt << " " << sapwoodcb[11]
    // << " " << sapwoodcb[4] << endl;
...
//   fluxes to zero
    ...
    if (0 == pdm)
    {
        ...
        else
        {
            ...
            {
                microbe.setKD(microbe.yrkd(nfeed, veg.yrltrc, veg.yrltrn, veg.cmnt));
    ...
    if (disturbflag == 1 && pdm == (disturbmonth - 1)) // agriculture
    {
        ...
        //  Original Approach
        //
        ag.conversion(veg.cmnt, y[I_ROOTC][ichrt] + y[I_LABILEC][ichrt], y[I_ROOTN][ichrt],
                      y[I_LEAFC][ichrt] / ag.getVCONVERT() + y[I_SAPWOODC][ichrt] + y[I_HEARTWOODC][ichrt] +
                          y[I_LABILEC][ichrt],
                      y[I_LEAFN][ichrt] / ag.getVCONVERT() + y[I_SAPWOODN][ichrt] + y[I_HEARTWOODN][ichrt],
                      y[I_LABILEN][ichrt], y[I_SOLC][ichrt], y[I_SOLN][ichrt], cwdloss, CYCLE);
        ...
        ag.standingdead(veg.cmnt, (y[I_ROOTC][ichrt]), (y[I_ROOTN][ichrt]), 0.0, 0.0, cwdloss);
    ...
    else if (disturbflag == 2 && pdm == (disturbmonth - 1)) // timber harvest clearcut
    {
        ...
        //   Original Approach
        //
        ag.conversion(veg.cmnt, timber * (y[I_LEAFC][ichrt] + y[I_ROOTC][ichrt] + y[I_LABILEC][ichrt]),
                      timber * (y[I_LEAFN][ichrt] + y[I_ROOTN][ichrt]),
                      timber * (y[I_SAPWOODC][ichrt] + y[I_HEARTWOODC][ichrt]),
                      timber * (y[I_SAPWOODN][ichrt] + y[I_HEARTWOODN][ichrt]), timber * y[I_LABILEN][ichrt],
                      timber * y[I_SOLC][ichrt], timber * y[I_SOLN][ichrt], cwdloss, CYCLE);
    ...
    else if (disturbflag == 3 && pdm == (disturbmonth - 1)) // fire, mid intensity
    {
        ...
        ag.conversion(veg.cmnt, dwood * y[I_ROOTC][ichrt], dwood * y[I_ROOTN][ichrt],
                      //              dwood*(y[I_SAPWOODC]+y[I_HEARTWOODC]+y[I_LABILEC]) + dleaf*y[I_LEAFC],
                      dwood * (y[I_SAPWOODC][ichrt] + y[I_HEARTWOODC][ichrt] + y[I_LABILEC][ichrt]) +
                          y[I_LEAFC][ichrt] * (dleaf / ag.getVCONVERT()),
                      //              dwood*(y[I_SAPWOODN]+y[I_HEARTWOODN]) + dleaf*y[I_LEAFN],
                      dwood * (y[I_SAPWOODN][ichrt] + y[I_HEARTWOODN][ichrt]) +
                          y[I_LEAFN][ichrt] * (dleaf / ag.getVCONVERT()),
                      dwood * y[I_LABILEN][ichrt], y[I_SOLC][ichrt], y[I_SOLN][ichrt], cwdloss, 1);
        //
        ////  overwrite the slash with same as above but add to standing dead
        ////
        ag.standingdead(
            veg.cmnt, dwood * y[I_ROOTC][ichrt], dwood * y[I_ROOTN][ichrt],
    ...
    else if (disturbflag == 4 && pdm == (disturbmonth - 1)) // hurricane-strength storm
    {
        ...
        ag.standingdead(veg.cmnt,
                        ...
                        0.99 * (y[I_SAPWOODN][ichrt] + y[I_HEARTWOODN][ichrt] + y[I_LABILEN][ichrt]), cwdloss);
        //
    ...
    //  stormoccur = 0;
    //  if(initFlag == 1 && pdm == 7 && pdyr > 1 && (veg.cmnt == 5 || veg.cmnt == 6 || veg.cmnt == 4 || veg.cmnt == 10
    //  || veg.cmnt == 11 || veg.cmnt == 12) && ag.state == 0)
    if (pdm == 7 && pdyr > 1 &&
        (veg.cmnt == 5 || veg.cmnt == 6 || veg.cmnt == 4 || veg.cmnt == 10 || veg.cmnt == 11 || veg.cmnt == 12) &&
        ag.state == 0)
    {
        ...
        if (tempret_storm != 0 && ((int)rand() % (tempret_storm - 1)) == 1)
        {
            ...
            //
            ag.standingdead(veg.cmnt, 0.07 * y[I_LEAFC][ichrt] + 0.04 * (y[I_ROOTC][ichrt]),
                            0.07 * y[I_LEAFN][ichrt] + 0.04 * (y[I_ROOTN][ichrt]),
                            (0.04 * (y[I_SAPWOODC][ichrt] + y[I_HEARTWOODC][ichrt] + y[I_LABILEC][ichrt])),
                            (0.04 * (y[I_SAPWOODN][ichrt] + y[I_HEARTWOODN][ichrt] + y[I_LABILEN][ichrt])), cwdloss);

            prevy[I_LABILEC][ichrt] = y[I_LABILEC][ichrt] =
                (1. - 0.04) * y[I_LABILEC][ichrt]; // for now, 0.5 of hurricane value
        ...
        if (tempret_hurr != 0 && ((int)rand() % (tempret_hurr - 1)) == 1)
        {
            ...
            ag.standingdead(veg.cmnt, 0.2 * y[I_LEAFC][ichrt] + 0.11 * (y[I_ROOTC][ichrt]),
                            0.2 * y[I_LEAFN][ichrt] + 0.11 * (y[I_ROOTN][ichrt]),
                            0.11 * (y[I_SAPWOODC][ichrt] + y[I_HEARTWOODC][ichrt] + y[I_LABILEC][ichrt]),
                            0.11 * (y[I_SAPWOODN][ichrt] + y[I_HEARTWOODN][ichrt] + y[I_LABILEN][ichrt]), cwdloss);

            prevy[I_LABILEC][ichrt] = y[I_LABILEC][ichrt] =
                (1. - 0.11) * y[I_LABILEC][ichrt]; // destruction values from Batista & Platt
    ...
    if (pdm == 7 && ag.state == 0 && pdyr > 1 && stormoccur == 0)
    {
        ...
        if ((dleaf != 0 || dwood != 0))
        {
            ...
            cout << "fire = " << pdyr << " " << veg.cmnt << " " << dwood << " " << dleaf << " " << lowi << " " << mixi
                 << " " << repi << " " << firemag << " " << vegmax << " " << replace << " " << veg.getVEGC() << " "
                 << rhmoist << " " << dq10 << endl;
            rflog1 << "fire = " << pdyr << " " << veg.cmnt << " " << dwood << " " << dleaf << " " << lowi << " " << mixi
                   << " " << repi << " " << firemag << " " << vegmax << " " << replace << " " << veg.getVEGC() << " "
                   << rhmoist << " " << dq10 << endl;
            fireoccur = 1;
            //       ag.setVCONVERT(1.0);
            ...
            ag.conversion(veg.cmnt,
                          ...
                          dwood * (y[I_SAPWOODC][ichrt] + y[I_HEARTWOODC][ichrt]) +
                              y[I_LEAFC][ichrt] * (dleaf / ag.getVCONVERT()),
                          //                     dwood*(y[I_SAPWOODN]+y[I_HEARTWOODN]) + dleaf*y[I_LEAFN],
                          dwood * (y[I_SAPWOODN][ichrt] + y[I_HEARTWOODN][ichrt]) +
                              y[I_LEAFN][ichrt] * (dleaf / ag.getVCONVERT()),
                          //                     dwood*y[I_LABILEN],
                          0.0, y[I_SOLC][ichrt], y[I_SOLN][ichrt], cwdloss, 1);
            //
            ////  overwrite the slash with same as above but add to standing dead
            ////
            ag.standingdead(
                ...
                //                       (1.0-ag.getVCONVERT())*(dwood*(y[I_SAPWOODN]+y[I_HEARTWOODN]+y[I_LABILEN]) ),
                (1.0 - ag.getVCONVERT()) * (dwood * (y[I_SAPWOODN][ichrt] + y[I_HEARTWOODN][ichrt])), cwdloss);
    ...
    dq10 = microbe.setDQ10LT(veg.cmnt, atms.getTAIR(), veg.getTOPTMIC());

    rhmoist = microbe.setRHMOIST(veg.cmnt, soil.getPCTFLDCAP(), soil.getPCTWILTPT(), soil.getPCTPOR(), y[I_VSM][ichrt],
                                 moistlim);
    ...
    if (0 == pdyr && 0 == pdm)
    {
        ...
        y[I_VSM][ichrt] = soil.updateRootZ(veg.cmnt, y[I_SM][ichrt], 100.0);
...
// Get environmental conditions for month "dm"
    ...
    if (1 == ag.state)
    {
        ...
        veg.phenology(ag.cmnt, atms.getTAIR(), atms.getTAIRD(), atms.getTAIRN(), soil.getAVLH2O(), soil.getAWCAPMM(),
                      atms.getPREC());
    ...
    else
    {
        ...
        veg.phenology(veg.cmnt, atms.getTAIR(), atms.getTAIRD(), atms.getTAIRN(), soil.getAVLH2O(), soil.getAWCAPMM(),
                      atms.getPREC());
    ...
    else
    {
        ...
        if (1 == ag.state && ag.getGROWDD() > ag.getGDDSEED(ag.cmnt) && atms.getTAIRN() <= ag.getTKILL(ag.cmnt))
        {
            ...
            refresh();
#endif
            ag.frostDamage(y[I_LEAFC][ichrt] + y[I_SAPWOODC][ichrt] + y[I_HEARTWOODC][ichrt] + y[I_ROOTC][ichrt] +
                               y[I_SEEDC][ichrt] + y[I_LABILEC][ichrt] - ag.getCROPSEEDC(ag.cmnt),
                           y[I_LEAFN][ichrt] + y[I_SAPWOODN][ichrt] + y[I_HEARTWOODN][ichrt] + y[I_ROOTN][ichrt] +
                               y[I_SEEDN][ichrt] + y[I_LABILEN][ichrt] - ag.getCROPSEEDSTON(ag.cmnt));
    ...
    if (ag.irrg1950flag == 1 && 3 == ag.state & ag.getGROWDD() >= ag.getGDDSEED(ag.cmnt) &&
        ag.getGROWDD() <= ag.getGDDHARVST(ag.cmnt))
    {
        if (atms.getPREC() < 200)
        {
            ag.irrigate = 200.0 - atms.getPREC();
            //    cout << "prec,irrigate = " << pdm << " " << pdyr << " " << atms.getPREC() << " " << ag.irrigate <<
            //    endl;
        }
    }
    ...
    {
        //   growth rate is 20% increase from 1993 to 2005 which is 12 years, so increse from 70% of deciduous to 84%,
        //   with corresponding decrease from 30% to 16% of the rest; GPP goes from 1380 to 1493, so nmax goes from 525
        //   to 680 in 12 year, which is 12.9/yr assume increase continues beyond 2005 for now
...
// Check mass balance
    ...
    // if(initFlag == 1) {cout << "year, month = " << pdyr << " " << pdm << " " << soil.getSONINP()  <<  " " <<
    // veg.getVEGNINP() << endl;} cout << "year, month = " << pdyr << " " << pdm << " " << soil.getSONINP()  <<  " " <<
    // veg.getVEGNINP() << endl; cout << "year, month = " << pdyr << " " << pdm << " " << y[I_LEAFC] << " " << y[I_AVLN]
    // << " " << ag.getIMMBADD() << endl; cout << "year, month = " << pdyr << " " << pdm << " " << veg.getVEGC() << " "
    // << veg.getGPP() << " " << veg.getVEGN() << " " << y[I_SOLC] << " " << y[I_SOLN] << " " << y[I_AVLN] <<  " " <<
    // veg.getSTRN() << " " <<  y[I_LABILEN] <<  endl;
    //   cout << "veggpp = " << veg.cmnt <<  " " << atms.getNDAYS(pdm) << "  " << atms.getCO2() << "  " << atms.getPAR()
    //   << " " << atms.getVPR() << " " << atms.getVPDD() << " " << atms.getDAYL() << " " << veg.pen.getKEXT(veg.cmnt)
    //   << " " <<   atms.getPREC() << " " << veg.getCMAX() << endl; cout << "veggpp = " << veg.cmnt <<  " " <<
    //   veg.getLAI() << " " <<  veg.getALLOCLC() << " " << veg.getRMLEAF() << " " << veg.getLTRLC() <<  " " <<
    //   pen.getLSCMIN(veg.cmnt) << endl;
    //`cout << "year, month = " << pdyr << " " << pdm << " " <<veg.getGPP() << " " << y[I_INGPP] << endl;
    // cout << "ROOTN = " << y[I_ROOTN] << " " << veg.getALLOCRN() << " " << veg.getNRESORBR() << " " << veg.getLTRRN()
    // << endl;

    // cout << "diag = " << pdyr << " " << pdm << " " << soil.getAVLH2O() << " " <<  soil.getAWCAPMM() << " " <<
    // veg.getPESOILW() << " " <<  atms.getPREC() << endl;
    ...
    //   cout << "diag = " << pdyr << " " << vegceq[pdyr] << " " << soilceq[pdyr] << " " << vegneq[pdyr] << " " <<
    //   soilneq[pdyr] << endl;

    //   }
    // cout << "time = " << pdyr << " " << pdm << endl;
    // cout << "water = " << pdyr << " " << pdm << " " << y[I_SM] <<  " " << soil.getAVLH2O() << " " <<
    // soil.getSNOWINF() << " " << atms.getRAIN() << " " << y[I_EET] << " " << y[I_RPERC] << " " << y[I_SPERC] << " " <<
    // soil.getSNOWINF()+atms.getRAIN()-y[I_EET]-y[I_RPERC]-y[I_SPERC] << " " << y[I_SM] - prevy[I_SM] << " " <<
    // y[I_PECAN] << " " << y[I_PESOIL] << " " << y[I_VSM] << endl;

    // cout << "leafc = " << y[I_LEAFC] << " " << y[I_ALLOCLC] << " " << y[I_RMLEAF] << " " << y[I_LTRLC] << " " <<
    // y[I_LEAFC] - prevy[I_LEAFC] << endl;

    // cout <<"vegc = " << y[I_SAPWOODC] << " " << y[I_HEARTWOODC] << " " << y[I_ROOTC] << " " << y[I_SEEDC] << " " <<
    // y[I_LABILEC] << " " << y[I_INGPP] << " " << y[I_GPP] << endl;

    wevapd[pdm] = pen.watev(atms.getNIRRN(), atms.getLWOUTD(), atms.getTAIRD(), atms.getVPDD(), atms.getWS10(),
                            soil.getRRUN(), soil.getSRUN(), pdm);

    wevapn[pdm] = pen.watev(0.0, atms.getLWOUTN(), atms.getTAIRN(), atms.getVPDN(), atms.getWS10(), soil.getRRUN(),
                            soil.getSRUN(), pdm);
    ...
    veg.setVEGC(y[I_LEAFC][ichrt] + y[I_SAPWOODC][ichrt] + y[I_HEARTWOODC][ichrt] + y[I_ROOTC][ichrt] +
                y[I_SEEDC][ichrt] + y[I_LABILEC][ichrt]);

    veg.setSTRN(y[I_LEAFN][ichrt] + y[I_SAPWOODN][ichrt] + y[I_HEARTWOODN][ichrt] + y[I_ROOTN][ichrt] +
                y[I_SEEDN][ichrt]);

    veg.setVEGN((veg.getSTRN() + y[I_LABILEN][ichrt]));

    // cout << "diag = " << veg.getVEGC() << " " << y[I_SOLC] << " " << ag.getSLASHC() << " " << ag.getVOLAC() << " " <<
    // ag.getCONVRTFLXC() << " " << ag.getSTANDDEADC() << endl; cout << "diag = " << veg.getVEGC() << " " << y[I_SOLC]
    // << " " << y[I_SLASHC] << " " << y[I_VOLAC] << " " << ag.getSCONVRTFLXC() << " " << y[I_STANDDEADC] << endl;
...
//   soil.setNINPUT( ( ag.fertn) );
    ...
    //   cout << "harvest = " << ag.getGROWDD() << " " << ag.getGDDHARVST(ag.cmnt) << " " << y[I_NPP] << " " <<
    //   y[I_SEEDC] << " " << y[I_ALLOCSEEDC] << " " << y[I_RMSEED] << " " << y[I_LTRSEEDC] << endl;
    ...
    // cout << "harvest = " << ag.state << " " << ag.getGROWDD() << " " << ag.getGDDHARVST(ag.cmnt) << " " << pdyr << "
    // " << pdm << endl;}
    if ((1 == ag.state) && ((ag.getGROWDD() >= ag.getGDDHARVST(ag.cmnt)) || (harcnt == 0 && pdm == 9)) &&
        (0 == ag.getFROSTFLAG()))
    //  if( (1 == ag.state) && (ag.getGROWDD() >= ag.getGDDHARVST(ag.cmnt)) && (0 == ag.getFROSTFLAG() ) )
    {
        //    ag.harvest( pdm, y[I_SEEDC], y[I_SEEDN], veg.getVEGC() - ag.getCROPSEEDC( ag.cmnt ), veg.getVEGN() -
        //    ag.getCROPSEEDSTON( ag.cmnt ) );
    ...
    if (3 == ag.state &&
        (ag.getGROWDD() >= ag.getGDDSEED(ag.cmnt) && ag.getGROWDD() <= ag.getGDDHARVST(ag.cmnt))) // turflawn
    {
        ...
        {
            frcab = (y[I_LEAFC][ichrt] + y[I_SAPWOODC][ichrt] + y[I_HEARTWOODC][ichrt]) /
                    (y[I_LEAFC][ichrt] + y[I_SAPWOODC][ichrt] + y[I_HEARTWOODC][ichrt] + y[I_ROOTC][ichrt]);
        }
        cabove =
            0.16 * (y[I_LABILEC][ichrt] * frcab + y[I_LEAFC][ichrt] + y[I_SAPWOODC][ichrt] + y[I_HEARTWOODC][ichrt]);
        nabove =
            0.16 * (y[I_LABILEN][ichrt] * frcab + y[I_LEAFN][ichrt] + y[I_SAPWOODN][ichrt] + y[I_HEARTWOODN][ichrt]);
...
// NOTINSHREE
    ...
    //  soil.setNLOST( (y[I_NLST]
    // cout << "nlost = " << soil.getNLOST() << " " << soil.getLCHDON() << " " << ag.getCONVRTFLXN() << " " <<
    // ag.getCROPRESIDUEFLXN() << endl;
    soil.setNLOST((soil.getNLOST()
                   //                  + nprod
                   //                  + y[I_LCHDON]
  ...
  soil.setNINPUT((ag.fertn));
    ...
    //  cout << "din = " << soil.getLCHDON() << " " << soil.getNLOST() << " " << y[I_NLST] << " " << soil.getLCHDOC() <<
    //  " " << microbe.getDOCDON(veg.cmnt) << endl;
    ...
    if (pdm == CYCLE - 1)
    {
        if ((veg.yralloclc + veg.yrallocsc + veg.yrallocrc + veg.yrallocseedc - veg.yrrmleaf - veg.yrrmsapwood -
             veg.yrrmroot) != ZERO)
        {
            veg.yrpleaf =
                (veg.yralloclc - veg.yrrmleaf) / (veg.yralloclc + veg.yrallocsc + veg.yrallocrc + veg.yrallocseedc -
                                                  veg.yrrmleaf - veg.yrrmsapwood - veg.yrrmroot);

            veg.yrpsapwood =
                (veg.yrallocsc - veg.yrrmsapwood) / (veg.yralloclc + veg.yrallocsc + veg.yrallocrc + veg.yrallocseedc -
                                                     veg.yrrmleaf - veg.yrrmsapwood - veg.yrrmroot);

            veg.yrproot =
                (veg.yrallocrc - veg.yrrmroot) / (veg.yralloclc + veg.yrallocsc + veg.yrallocrc + veg.yrallocseedc -
                                                  veg.yrrmleaf - veg.yrrmsapwood - veg.yrrmroot);

            veg.yrpseed = (veg.yrallocseedc) / (veg.yralloclc + veg.yrallocsc + veg.yrallocrc + veg.yrallocseedc -
                                                veg.yrrmleaf - veg.yrrmsapwood - veg.yrrmroot);
        }
        else
        {
    ...
    veg.setRLTRC(veg.getRLTRC() * avgfac + (y[I_LTRLC][ichrt] + y[I_LTRSC][ichrt] + y[I_LTRHC][ichrt] +
                                            y[I_LTRRC][ichrt] + y[I_LTRSEEDC][ichrt]) *
                                               (1.0 - avgfac));
    ...
    mdemandc =
        y[I_ALLOCLC][ichrt] + y[I_ALLOCSC][ichrt] + y[I_ALLOCRC][ichrt] + y[I_ALLOCSEEDC][ichrt] + y[I_RVGRW][ichrt];
    ...
    else
    {
        if (0 == ag.state)
        {
            veg.resetNEWTOPT(veg.cmnt, atms.getTAIRD(), veg.getRTAIRPHI(), veg.getRPHI());
        }
        else
        {
            veg.resetNEWTOPT(ag.cmnt, atms.getTAIRD(), veg.getRTAIRPHI(), veg.getRPHI());
        }
    }
    ...
    if ((CYCLE - 1) == pdm)
    {
        ...
        //      availnp = (ag.fertn+veg.getVEGNINP()+atms.getNDEP()/12000.0)/ntot;
              ...
              {
        //        cout << "nin > nout " << soil.yrnin - soil.yrnlost << " " << sonp << " " << availnp << " " << vegnp <<
        " " << ntot<< endl;
        //        cout << "compare = " << soil.getSONINP() << " " << y[I_NFIXN] << " " << veg.getVEGNINP() << " " <<
        y[I_NFIXS] << endl;
        //      cout << "removing N = " << soil.yrnin << " " << soil.yrnlost << " " << sonp << " " << availnp << " " <<
        vegnp << endl; y[I_SOLN] = y[I_SOLN] - (soil.yrnin - soil.yrnlost)*sonp; y[I_AVLN] = y[I_AVLN] - (soil.yrnin -
        soil.yrnlost)*availnp; y[I_LABILEN] = y[I_LABILEN] - (soil.yrnin - soil.yrnlost)*vegnp; veg.setVEGN(
        (veg.getSTRN() + y[I_LABILEN]) );
...
int Ttem45::testEquilibrium(const int &pdyr, const int &ichrt, const int &nyears, const double &vegceq,
                            const double &soilceq, const double &vegneq, const double &soilneq)
{
    //  cout << "diag = " << pdyr << " " << vegceq[pdyr] << " " << soilceq[pdyr] << " " << vegneq[pdyr] << " " <<
    //  soilneq[pdyr] << " " << vegceq[pdyr]+soilceq[pdyr] << " " << (vegceq[pdyr-39]+soilceq[pdyr-39]) << " " <<
    //  vegceq[pdyr-39] << " " <<soilceq[pdyr-39] << endl;

    // cout << "testEquilibrium " << pdyr <<  endl;
    if (pdyr >= nyears + strteq)
    {
        if (0 == nfeed && 0 == rheqflag &&
            wtol >= fabs(atms.yrrain + atms.yrsnowfall - soil.yreet - soil.yrrrun - soil.yrsrun) &&
        ...
        if (0 == nfeed && 1 == rheqflag &&
            ...
            && (ctol >= fabs((veg.getVEGC() + y[I_SOLC][ichrt]) - (vegceq + soilceq))) &&
            (ctol >= fabs((veg.getVEGC() - vegceq))) && (ctol >= fabs((y[I_SOLC][ichrt] - soilceq))))
        {
            return 1;
        }

        // cout << "equilibrium = " << fabs( soil.yrnin - soil.yrnlost ) << " " <<
        // (veg.getVEGN()+y[I_SOLN])-(vegneq+soilneq) << " " << (veg.getVEGN()-vegneq) << " " << (y[I_SOLN]-soilneq) <<
        // endl; cout << "equil2 = " << veg.getVEGC() << " " << vegceq  << endl;
        if (ag.state == 0)
        {
            if (1 == nfeed && 1 == rheqflag &&
                ...
                && (ntol >= fabs(soil.yrnin - soil.yrnlost)) &&
                (ntol >= fabs((veg.getVEGN() + y[I_SOLN][ichrt]) - (vegneq + soilneq))) &&
                (ntol >= fabs((veg.getVEGN() - vegneq))) && (ntol >= fabs((y[I_SOLN][ichrt] - soilneq)))

                && (ctol >= fabs((veg.getVEGC() + y[I_SOLC][ichrt]) - (vegceq + soilceq))) &&
                (ctol >= fabs((veg.getVEGC() - vegceq))) && (ctol >= fabs((y[I_SOLC][ichrt] - soilceq))))
            {
                // cout << "uhoh = " << veg.getVEGC() << " " << vegceq << " " << ctol << " " << veg.getVEGC()-vegceq <<
                // endl;
                return 1;
            }
        }
        else if (ag.state == 1)
        {
            if (1 == nfeed && 1 == rheqflag &&
                ...
                && (ntol >= (veg.getVEGN() + y[I_SOLN][ichrt]) - (vegneq + soilneq)) &&
                (ntol >= (veg.getVEGN() - vegneq)) && (ntol >= (y[I_SOLN][ichrt] - soilneq))

                && (ctol >= (veg.getVEGC() + y[I_SOLC][ichrt]) - (vegceq + soilceq)) &&
...
void Ttem45::updateYearSummary(const int &pdm, const int &ichrt)
{
    ...
    veg.yrrmaint = veg.yrrmaint * (1.0 - favg) +
                   12.0 * (y[I_RMLEAF][ichrt] + y[I_RMSAPWOOD][ichrt] + y[I_RMROOT][ichrt] + y[I_RMSEED][ichrt]) * favg;
    ...
    veg.yrltrc =
        veg.yrltrc * (1.0 - favg) +
        12.0 * (y[I_LTRLC][ichrt] + y[I_LTRSC][ichrt] + y[I_LTRHC][ichrt] + y[I_LTRRC][ichrt] + y[I_LTRSEEDC][ichrt]) *
            favg;
    ...
    veg.yrnrsorb =
        veg.yrnrsorb * (1.0 - favg) +
        12.0 * (y[I_NRESORBL][ichrt] + y[I_NRESORBS][ichrt] + y[I_NRESORBR][ichrt] + y[I_NRESORBSEED][ichrt]) * favg;

    veg.yrltrn =
        veg.yrltrn * (1.0 - favg) +
        12.0 * (y[I_LTRLN][ichrt] + y[I_LTRSN][ichrt] + y[I_LTRHN][ichrt] + y[I_LTRRN][ichrt] + y[I_LTRSEEDN][ichrt]) *
            favg;
    ...
    // cout << "nvals = " << soil.yrnin << " " << soil.yrnlost << " " << soil.getNINPUT() << " " << soil.getNLOST() <<
    // endl;
    ...
    {
        ag.yrgrowdd =
            ag.yrgrowdd * (1.0 - favg) + 12.0 * (atms.getTAIR() - ag.getGDDMIN(ag.cmnt)) * atms.getNDAYS(pdm) * favg;
    }
    else
    {
