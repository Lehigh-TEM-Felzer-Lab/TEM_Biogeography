...
# Import Data
current = pd.read_csv(
    "./biogeo/input/current_land_cover.csv", names=["LON", "LAT", "TYPE"]
)
current_modelled = pd.read_csv(
    "./historical/modeled_current_land_cover.csv", names=["LON", "LAT", "TYPE"]
)
future_model_rcp45 = pd.read_csv(
    "./biogeo/output_bakeoff/end.shift", names=["LON", "LAT", "TYPE"]
)
future_model_rcp85 = pd.read_csv(
    "runs/biogeo/output_bakeoff/end.csv",
    names=["LON", "LAT", "TYPE"],
)
...
for i, data in enumerate(data_list):
    ...
    ax.add_feature(cfeature.STATES, linestyle="-", alpha=0.2)
