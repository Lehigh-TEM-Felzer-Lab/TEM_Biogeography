...
if original_lc:
    ...
    historical_vegc_path_or = "historical/runs/VEGC.SUMMARY_ORG"
    historical_npp_path_or = "historical/runs/NPP.SUMMARY_ORG"
    historical_nep_path_or = "historical/runs/NEP.SUMMARY_ORG"
    historical_soilorgc_path_or = "historical/runs/SOILORGC.SUMMARY_ORG"
...
if original_lc == False:
    # Historical Path
    historical_vegc_path = "historical/runs/VEGC.SUMMARY"
    historical_npp_path = "historical/runs/NPP.SUMMARY"
    historical_nep_path = "historical/runs/NEP.SUMMARY"
    historical_soilorgc_path = "historical/runs/SOILORGC.SUMMARY"
...
def add_missing_potveg_entries(df, years, potveg_values, year_reference):
    ...
    for year in years:
        ...
        for potveg in missing_potveg:
            potveg_description = description.get(
                potveg, "Unknown Description"
            )  # Default to "Unknown Description" if not found
...
def calculate_difference(rcp_df, historical_df):
    ...
    for year in historical_years:
        historical = add_missing_potveg_entries(
            historical, historical_years, unique_potveg, year
        )
    ...
    merged_df = pd.merge(
        historical_agg, rcp_agg, on="POTVEG", suffixes=("_HIST", "_RCP")
    )

    # Calculate the difference in totals and the standard deviation of the difference
    merged_df["DIFFERENCE"] = (
        merged_df["TOTFORECOZONE_RCP"] - merged_df["TOTFORECOZONE_HIST"]
    )
    merged_df["STNDEV_DIFF"] = np.sqrt(
        merged_df["STNDEV_HIST"] ** 2 + merged_df["STNDEV_RCP"] ** 2
    )

    # Calculate the standard error of the mean (SEM) for the difference
    merged_df["STDERR"] = np.sqrt(
        (merged_df["STNDEV_HIST"] ** 2 / merged_df["count_HIST"])
        + (merged_df["STNDEV_RCP"] ** 2 / merged_df["count_RCP"])
    )
...
data = [
    concatinated["rcp_vegc"],
    concatinated["rcp_soilorgc"],
    concatinated["rcp_npp"],
    concatinated["rcp_nep"],
]
...
for i, ax in enumerate(axs.flatten()):
    if i < len(data):
        ...
        for pft_value, row in data[i].iterrows():
            position = order_list.index(
                row["PFT"]
            )  # Fetching position based on order_list
            ax.errorbar(
                x=position,
                y=row["DIFFERENCE"],
        ...
        legend_patches.append(
            mpatches.Patch(
                facecolor="white", edgecolor="black", hatch="//", label="RCP 8.5"
            )
        )
        legend_patches.append(
            mpatches.Patch(
                facecolor="white", edgecolor="black", hatch="-", label="RCP 4.5"
            )
        )
