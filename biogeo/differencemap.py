...
end_45 = pd.read_csv(
    "/users/jkodero/scratch/tem/4.5/runs/biogeo/output_bakeoff/end.shift",
    names=["LON", "LAT", "POTVEG"],
)
end_45 = end_45.drop_duplicates()
end_45 = end_45.query("POTVEG != 13 and POTVEG !=15")

end_85 = pd.read_csv(
    "/users/jkodero/scratch/tem/8.5/runs/biogeo/output_bakeoff/end.shift",
    names=["LON", "LAT", "POTVEG"],
)
end_85 = end_85.drop_duplicates()
end_85 = end_85.query("POTVEG != 13 and POTVEG !=15")
...
def remove_outliers(df):
    ...
    outliers = (df["PERCENT_DIFFERENCE"] < lower_cutoff) | (
        df["PERCENT_DIFFERENCE"] > upper_cutoff
    )
...
soilorgc_difference_45 = remove_outliers(
    calculate_difference(soilorgc_df_historical, soilorgc_45)
)
...
soilorgc_difference_85 = remove_outliers(
    calculate_difference(soilorgc_df_historical, soilorgc_85)
)
...
vgc_85 = pd.read_csv(
    "/users/jkodero/scratch/tem/archive/runs/8.5/vegc_difference.csv", header=0
)
npp_85 = pd.read_csv(
    "/users/jkodero/scratch/tem/archive/runs/8.5/npp_difference.csv", header=0
)
...
subplot_titles = [
    "a) Vegetation Carbon RCP 4.5 ",
    " b) Net Primary Productivity RCP 4.5",
    " c) Vegetation Carbon RCP 8.5",
    " c) Net Primary Productivity RCP 8.5",
]
